...
var webConfigurationEngine = {
	...
	buildMenuHTML : function () {
		...
		var homeDashboardTab = $.grep(access_profile.user_functional_access, function (element, index) {
							return element.child_level_no == "0" && element.child_feature_id_or_group.includes("DASHBOARD") && element.feature_access == "true"
						});
		if (homeDashboardTab.length != 0) {
				for (var index = 0; index < homeDashboardTab.length; index ++) {
					innerHTMLString += "<li data-screenid = '" + homeDashboardTab[index].child_screen_id + "'>Dashboard</li>";
				}
			};
