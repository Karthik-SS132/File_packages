/****** Object:  StoredProcedure [dbo].[sp_generate_customer_survey]    Script Date: 15/2/2023 10:48:00 AM ******/
	...
	declare @p_survey_status varchar(2),
		...
		begin
			...
			(
			  ...
			  system_user_generation_ind,job_order_no,act_start_date,
			  last_update_id
			)
			select  cr.company_id, cr.country_code, @o_survey_ref_no, 
					...
					cr.customer_contact_name,cr.customer_contact_no,
					cr.customer_contact_email_id,'U',@i_call_jo_project_ref_no,act_start_date,
					@i_user_id		 
			  ...
			  select @i_client_id, @i_country_code, @o_survey_ref_no, 'WEB',
					'OPEN',
					0, 1, SYSDATETIMEOFFSET(), 'O','', @p_employee_id, @i_user_id
