/* Mservice Common Scripts - Version 1.0.04_accts - 16/03/2020 */
...
var webConfigurationEngine = {
	...
	buildMenuHTML : function () {
		...
		for (functionalAccessCounter = 0; functionalAccessCounter < access_profile.user_functional_access.length; functionalAccessCounter++) {
			sortedParentDisplayOrder = $.grep(access_profile.user_functional_access, function (element, index) {
					return element.parent_display_order == functionalAccessCounter.toString();
				});
			...
			if (configurationNode != undefined) {
				...
				for (contentNodesCounter = 0; contentNodesCounter < contentNodes.length; contentNodesCounter++) {
					...
					for (fieldNodesCounter = 0; fieldNodesCounter < fieldNodes.length; fieldNodesCounter++) {
						...
						} else if (fieldType == "w_mobilenumber") {
							$("#" + screenID + "_" + fieldID).initializeWMobilenumber(initObject);
						}
					}
				}
			}
	...
	createFieldGroup : function (fieldGroupObject) {
		...
		descriptionElement = "";
		if (fieldGroupObject.type == "w_dropdownlist" || fieldGroupObject.type == "w_combobox" || fieldGroupObject.type == "w_searchbox" || fieldGroupObject.type == "w_mobilenumber") {
	...
	getUiConfigurationObject : function (currentScreenID) {
		...
		if (currentScreenID != undefined) {
			...
			if (returnValue == undefined) {
				$.ajax({
					url : "../configuration/ui/ui_" + currentScreenID + "_" + login_profile.client_id + "_" + login_profile.country_code + ".xml",
					async : false,
					dataType : "xml",
					cache : true
				}).done(function (data) {
...
var webNavigationController = {
	...
	gotoNextScreen : function (navigationObject) {
		...
		ga('set', 'page', navigationObject.screenID);
		ga('send', 'pageview');
	...
	applyConfiguredLabels : function (eventVerb) {
		...
		if (currentScreenID != undefined) {
			...
			if (grepReturnValue == undefined) {
				$.ajax({
					url : "../configuration/label/label_" + currentScreenID + "_" + login_profile.client_id + "_" + login_profile.country_code + "_" + login_profile.locale_id + ".xml",
					async : false,
					dataType : "xml",
					cache : true
				}).done(function (data) {
		...
		if ($(wButton).attr("data-button-group") == "crud") {
			...
			if (screenObject.variable.standard.valueChangeIndicator) {
				if (confirm("Do you want to cancel the changes you made ?")) {
					webNavigationController.gotoPreviousScreen(screenObject.variable.standard.popupIndicator);
					if (screenID == "manage_change_password" && home.variable.displaySingleMenu == true) {
