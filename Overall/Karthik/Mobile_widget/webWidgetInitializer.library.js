/* Mservice Common Scripts - Version 1.2.01 - 10/01/20 */
...
var webWidgetInitializer = {
	initializeWAttachment : function (initObject) {
		...
		multipleValue,
		validateFileSize;
		screenObject = eval(initObject.screenID);
		multipleValue = true;
	...
	initializeWCombobox : function (initObject) {
		...
		$("#" + initObject.fieldID).kendoComboBox({
			...
			delay : delayValue,
			//noDataTemplate: 'Please type some value to Search.',
	...
	initializeWGrid : function (initObject) {
		...
		if (typeof(initObject.toolbar) == "object") {
			...
			gridToolbar = kendo.template(toolbarElement.outerHTML.replace("</div>", exportElement + "</div>"));
		} else {
			toolbarElement = "<div class = 'toolbar'>" + exportElement + "</div>";
				...
				if (event.item.childElementCount == 1) {
					...
					if (newTemplateScreen != undefined) {
						...
						if (currentScreenID != undefined && currentScreenID != "home_container") {
							currentScreenObject = eval(webNavigationController.getCurrentScreenID());
						...
						if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_" + nextScreenID + ".js"])) {
							...
							nextScreenObject = eval(nextScreenID);
							try {
						...
						if (LoadJSScripts(["./webui/scripts/js/fn_" + screenName + ".js"])) {
							if (eval("fn_" + screenName + "_loadScripts()")) {
	...
	initializeWMultiselect : function (initObject) {
		...
		if (actualDefaultValue != undefined && actualDefaultValue != "") {
			webWidgetInitializer.variable.defaultValueDisplay[initObject.fieldID] = true;
			for (defaultValueCounter = 0; defaultValueCounter < actualDefaultValue.length; defaultValueCounter++) {
				defaultValueObject = {};
				defaultValueObject[initObject.dataValueField] = actualDefaultValue[defaultValueCounter];
				defaultValueObject[initObject.dataTextField] = actualDefaultValue[defaultValueCounter];
				$("#" + initObject.fieldID).data("kendoMultiSelect").dataSource.data().push(defaultValueObject);
			};
	...
	initializeWPopup : function (initObject) {
		...
		nextScreenObject = eval(initObject.nextScreenID);
	...
	initializeWMobilenumber : function (initObject) {
		var screenObject,
		actualDefaultValue,
		templateValue,
		placeholderValue,
		...
		delayValue;
		screenObject = eval(initObject.screenID);
		if(mserviceUtilities.getActualValue(initObject.defaultValue) != undefined && mserviceUtilities.getActualValue(initObject.defaultValue) != ""){
			actualDefaultValue = mserviceUtilities.getActualValue(initObject.defaultValue).split('-')[0];
		};
		templateValue = "${data." + initObject.dataValueField + "}" + " - " + "${data." + initObject.dataTextField + "}";
		if (initObject.template == "code") {
			templateValue = "${data." + initObject.dataValueField + "}";
		...
		$("#" + initObject.fieldID).parent().append("<input id='" + initObject.fieldID + "_textentry" + "' class='k-textbox' type='number' min='0' maxLength='15' onkeypress='return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))' oninput='javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);' data-widget-subtype='textentry'/>");
		$("#" + initObject.fieldID).data("kendoComboBox").options.dataTextField = initObject.dataTextField;
		webWidgetInitializer.variable.defaultValueDisplay[initObject.fieldID] = false;
		if (actualDefaultValue != undefined && actualDefaultValue != "") {
			webWidgetInitializer.variable.defaultValueDisplay[initObject.fieldID] = true;
			...
			if(mserviceUtilities.getActualValue(initObject.defaultValue).split('-')[1] != "undefined"){
				$("#" + initObject.fieldID + "_textentry").val(mserviceUtilities.getActualValue(initObject.defaultValue).split('-')[1]);
			}
		};
		$("#" + initObject.fieldID).data("kendoComboBox").bind("change", function (event) {
			if ($("#" + initObject.fieldID).data("kendoComboBox").dataItem() != undefined) {
		...
		$("#" + initObject.fieldID).attr("data-widget-type", "w_mobilenumber");
	...
	variable : {
		defaultValueDisplay : {},
		userSelected : {},
		widgetSelector : "[data-widget-type = 'w_attachment'], [data-widget-type = 'w_checkbox'], [data-widget-type = 'w_combobox'], [data-widget-type = 'w_searchbox'], [data-widget-type = 'w_datepicker'], [data-widget-type = 'w_datetimepicker'], [data-widget-type = 'w_displayarea'], [data-widget-type = 'w_dropdownlist'], [data-widget-type = 'w_multiselect'], [data-widget-type = 'w_numerictextbox'], [data-widget-type = 'w_textarea'], [data-widget-type = 'w_textbox'], [data-widget-type = 'w_mobilenumber']"
	}
};
$.fn.extend({
	disable : function () {
		...
		} else if ($(this).attr("data-widget-type") == "w_mobilenumber") {
			$(this).data("kendoComboBox").enable(false);
	...
	getVal : function () {
		...
		if ($(this).attr("data-widget-type") == "w_textbox") {
			...
			if($(this).data("kendoComboBox").value() != ""){
	...
	initializeWMobilenumber : function (initObject) {
		initObject.fieldID = $(this).attr("id");
		return webWidgetInitializer.initializeWMobilenumber(initObject);
	},
