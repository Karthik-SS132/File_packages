/* 
 * This file contains invocation code snippets for executing the service report_sla_performance_for_scall. These code snippets have been organized as three
 * seperate functions. Include these snippet file reference in the html/aspx page or javascript file from where the service report_sla_performance_for_scall 
 * would be invoked. Substitute USER CONTROL tokens in these snippets, with corresponding user control references of the html/aspx page.
 */

//Code for executing the service report_sla_performance_for_scall
function executeService_report_sla_performance_for_scall()
{
    var targetURL =  getWebserverpath() + "/mservice/report_sla_performance_for_scall.aspx";
    
    //Prepare report_sla_performance_for_scall service request and invoke service 
    var responseXML = executeService(targetURL, prepareServiceRequestData_report_sla_performance_for_scall());
    ...
    {
       return processServiceResponseData_report_sla_performance_for_scall(responseXML);
      //  return true;
    }
    //return false;
}

//Code for preparing input data for the service report_sla_performance_for_scall
/* 
 ...
 */
function prepareServiceRequestData_report_sla_performance_for_scall()
{
    var serviceDetails;
    ...
    serviceDetails = serviceDetails + "<p_assigned_to_empid_filter>" + getXmlString(array[12]) + "</p_assigned_to_empid_filter>";  // Unicode string
    ...
    serviceDetails = serviceDetails + "<p_reporting_to_empid_filter>" + getXmlString(array[15]) + "</p_reporting_to_empid_filter>";  // Unicode string
    ...
    serviceDetails = serviceDetails + "<p_mapped_to_empid_filter>" + getXmlString(array[17]) + "</p_mapped_to_empid_filter>";  // Unicode string
    serviceDetails = serviceDetails + "</inputparam>";
...
//Code for processing output data from the service report_sla_performance_for_scall
/*
 ...
 */
function processServiceResponseData_report_sla_performance_for_scall(xmlDoc)
{
    var responseXMLNode = xmlDoc.documentElement.childNodes[0].childNodes;
    var outputparam_detailcount = 1;
    var first_outputparam_detail_item = true;
    var sc_sla_perf="";
    sc_sla_perf += "<?xml version=\"1.0\" encoding=\"UTF-8\"?><list>";
    ...
    for (var toplevelChildi = 0; toplevelChildi < responseXMLNode.length; toplevelChildi++)
    {
        if (responseXMLNode[toplevelChildi].nodeName == "outputparam_detail")
        {
          ...
          //  document.getElementById('USER CONTROL').rows[outputparam_detailcount].cells[COLNO].innerHTML = responseXMLNode[toplevelChildi].childNodes[0].lastChild.nodeValue; // p_sla_scall_perfmetrics_xml
           // outputparam_detailcount++;
		   sc_sla_perf = sc_sla_perf+responseXMLNode[toplevelChildi].childNodes[0].lastChild.nodeValue;
        }
    }
	sc_sla_perf = sc_sla_perf+"</list>";
	return sc_sla_perf;
}
