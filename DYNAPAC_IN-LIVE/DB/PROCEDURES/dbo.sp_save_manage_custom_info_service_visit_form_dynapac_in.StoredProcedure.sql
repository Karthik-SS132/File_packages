/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_service_visit_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_service_visit_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_service_visit_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_service_visit_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'service_visit_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'service_visit_form')
	...
	select @p_event_longitude = json_value(@i_inputparam_header_xml, '$.event_longitude')
			...
			@p_update_status varchar(5), @p_call_ref_no nvarchar(20), @p_error_no varchar(15), @p_enquiry_tag nvarchar(20), @p_designation nvarchar(60),
			...
			@p_problem_description nvarchar(1000),@p_dealer_code_oemref nvarchar(20),@p_user_type varchar(15), @p_customer_id varchar(15), @p_customer_id_new varchar(15), @p_asset_id nvarchar(30), @p_equipment_id nvarchar(30),
			@p_customer_name nvarchar(500),@p_customer_contact_no nvarchar(20),@p_customer_address nvarchar(500),@p_customer_city nvarchar(500),
			@p_company_location_code nvarchar(8),@p_cust_loc nvarchar(8),@p_asset_loc nvarchar(8),@p_customer_id_gen  varchar(15), @p_customer_contact_name nvarchar(60)



			select @p_user_type = json_value(@i_inputparam_header_xml, '$.user_type_savedetails')
			...
			select @p_customer_name = json_value(@i_inputparam_header_xml, '$.customer_name_savedetails') 
			select @p_customer_address = json_value(@i_inputparam_header_xml, '$.address_savedetails') 
			select @p_customer_city = json_value(@i_inputparam_header_xml, '$.city_new_savedetails') 
			select @p_asset_id =json_value(@i_inputparam_header_xml, '$.asset_id_savedetails')
			select @p_customer_contact_name =json_value(@i_inputparam_header_xml, '$.cust_contact_person_savedetails') 
			select @p_customer_contact_no = json_value(@i_inputparam_header_xml, '$.phone_no_savedetails') 
			select @p_customer_contact_email = json_value(@i_inputparam_header_xml, '$.email_id_savedetails') 
			select @p_requested_date = json_value(@i_inputparam_header_xml, '$.event_date') 
			...
			select @p_problem_description = isnull(json_value(@i_inputparam_header_xml, '$.purpose'),'')
			...
			select @p_cust_loc = json_value(@i_inputparam_header_xml, '$.customer_loc_savedetails')
			select @p_asset_loc = json_value(@i_inputparam_header_xml, '$.city_new_savedetails') 
			...
			if @p_user_type = 'SEPROVISIT'
			begin 		
				declare @p_state_desc nvarchar(10)

				select @p_customer_contact_name = json_value(@i_inputparam_header_xml, '$.cust_contact_person_new_savedetails')
				select @p_customer_name = json_value(@i_inputparam_header_xml, '$.customer_name_new_savedetails')
				select @p_customer_address = json_value(@i_inputparam_header_xml, '$.address_new_savedetails')
				select @p_customer_city = json_value(@i_inputparam_header_xml, '$.city_new_savedetails')
				select @p_customer_contact_no = json_value(@i_inputparam_header_xml, '$.phone_no_new_savedetails')
				select @p_customer_state_code = json_value(@i_inputparam_header_xml, '$.state_new_savedetails')
				select @p_customer_id_new = json_value(@i_inputparam_header_xml, '$.customer_id_new_savedetails')
				
				/* Check if the provided customer name and address already exists */
				select @p_state_desc = state_code
				from state
				where country_code = @i_country_code
					and state = @p_customer_state_code
				...
				execute sp_save_manage_customer_master @i_session_id,@i_user_id,@i_client_id,@i_locale_id,@i_country_code,
													   ...
													   @p_customer_state_code,'in',@p_pincode,'','','','',@p_customer_contact_name,
													   @p_customer_contact_no,@p_customer_contact_email, '','','',
														'<inputparam></inputparam>','00000000-0000-0000-0000-000000000000','A',
														@p_customer_id_gen output, @o_update_status output, @i_error_msg output
             
				set @p_customer_id = @p_customer_id_gen
				...
				begin
					...
					if (isnull(@p_cust_loc,'') != '')
						begin
							select @p_customer_location_code = @p_cust_loc
						end
					else
						begin
			...
			select @p_customer_location_code =isnull(( select top(1)location_code
					from customer_location
					where company_id = @i_client_id
						and country_code = @i_country_code
						and customer_id = @p_customer_id
		   ...
		   select @p_udf_xml = '<inputparam>
			...
			execute sp_save_manage_call_register @i_client_id, @i_country_code, @i_session_id, @i_user_id, @i_locale_id, 
							@p_customer_id, @p_asset_id,@p_asset_loc, @p_equipment_id,
							@p_problem_description, 'LOW', @i_user_id, @p_logged_on_date, @p_logged_on_hour, @p_logged_on_minute,
							@p_customer_location_code, @p_org_lvl_no, @p_org_lvl_code, 'VII', @p_user_type,
							'', @p_company_location_code, @p_customer_contact_name, @p_customer_contact_no, @p_customer_contact_email,
							'NB', 0, 0, 0, 0, 'INR', '', '0', '', '', @p_udf_xml,
							'A','00000000-0000-0000-0000-000000000000', @o_update_status output, @p_call_ref_no output, @p_error_no output
	
				  /* Updating additional value*/
						update  call_register set
								customer_name  = @p_customer_name, customer_address = @p_customer_address,
					...
					set organogram_level_code  = @p_dealer_code_oemref
					where company_id = @i_client_id
					and country_code = @i_country_code 
					and call_ref_no = @p_call_ref_no
