/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_dynapac_prospect_visit_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_dynapac_prospect_visit_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_dynapac_prospect_visit_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_dynapac_prospect_visit_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'dynapac_prospect_visit_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'dynapac_prospect_visit_form')
		...
		declare @p_usr_type varchar(15), @p_cust_id varchar(15), @p_cust_id_new varchar(15), @p_ast_id nvarchar(30), @p_equipmnt_id nvarchar(30),
			...
			begin 		
				...
				execute sp_save_manage_customer_master @i_session_id,@i_user_id,@i_client_id,@i_locale_id,@i_country_code,
							...
							begin
								select  @i_error_msg = '{"code":"failed to create a new customer"}'
								return
							end
			...
			else if @p_usr_type in ('DEMOVISIT','FINVISIT','LEADFOLWUP')
			begin 
		   ...
		   select @p_udf_xml = '<inputparam>
			...
			execute sp_save_manage_call_register @i_client_id, @i_country_code, @i_session_id, @i_user_id, @i_locale_id, 
				 ...
				 if @p_usr_type in ('DEMOVISIT','FINVISIT','QFOLWUP','LEADFOLWUP')
				 begin 
				 ...
				 delete call_user_attachments 
							where  company_id = @i_client_id
										and country_code = @i_country_code 
										and attachment_file_name like '%dynapac_prospect_visit%' 
