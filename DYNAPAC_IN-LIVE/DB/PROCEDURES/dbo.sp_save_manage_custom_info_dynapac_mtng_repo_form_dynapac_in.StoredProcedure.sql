/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_dynapac_mtng_repo_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_dynapac_mtng_repo_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_dynapac_mtng_repo_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_dynapac_mtng_repo_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'dynapac_mtng_repo_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'dynapac_mtng_repo_form')
	...
	if (@p_file_name like '%dynapac_mtng_repo_form%')
		begin
			...
			IF not exists (select 1 from call_register_actions   
				...
				begin
					...
					p_udf_analysis_code2,p_udf_analysis_code3
						from #part_temp
						where p_file_name like '%dynapac_mtng_repo%' 
							...
							BEGIN
								...
								/* form comments from visit report */
									...
									select @i_client_id, @i_country_code, @p_call_ref_no_v,
											...
											and country_code = @i_country_code
											and call_ref_no  = @p_transaction_ref_no
											and eventverb_id like '%Sales Flow Form%'
