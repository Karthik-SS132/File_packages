/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_marini_inspec_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_marini_inspec_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_marini_inspec_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_marini_inspec_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'marini_inspec_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'marini_inspec_form')
	...
	select isnull(part_no,''),isnull(part_desc,''),isnull(part_qnty,'') from OPENJSON(@i_inputparam_header_xml,'$.parts') 
	with (
	part_no int '$.parts_savedetails',
	part_desc nvarchar(50) '$.descrip_savedetails',
	part_qnty int '$.qty_savedetails'
	)
	
	if (@p_file_name like '%marini_inspec_form%')
		begin
		...
		select @p_notification_xml = '<notification_info>'+
				...
				@i_locale_id , @i_country_code , 'CALL_EMAIL_PAV_PDF' ,@p_notification_xml, @i_user_id, @p_notification_id OUTPUT	
			...
			IF not exists (select 1 from call_register_actions  
							  ...
							  and	 action_category	= 'PARTREP'
							  and	 action_type		= 'PARTREP')
					BEGIN
						insert into call_register_actions
						( company_id,country_code,call_ref_no,action_category,action_type,product_code,udf_char_1,udf_char_3,
						udf_char_4,udf_analysis_code1,product_sub_code,requirement,no_of_units,uom_code,last_update_id)
						select 
							@i_client_id,@i_country_code,@p_transaction_ref_no,'PARTREP','PARTREP',part_no,'NA','NA','NA','NA','NA',		
							isnull(part_desc,''),part_qnty,'NOS',@i_user_id
							from #part_temp
					END
