		...
		and c.notification_mode = d.notification_mode
			...
			begin
				...
				begin
					...
					where a.company_id = @i_client_id
					  and a.country_code = @i_country_code
					  and a.employee_id = ( select top(1) ap.by_employee_id 
											from asset_pwclaim_status_event_log ap
											where ap.company_id = @i_client_id
											  and ap.country_code = @i_country_code
											  and ap.eventverb_id = 'PWCLAIMSUBMITFORAPPROVAL'
											  and ap.pwclaim_ref_no = (select ah.pwclaim_ref_no from asset_pwclaim_header ah
																		where ah.company_id = @i_client_id
																			and ah.country_code = @i_country_code
																			and ah.txn_ref_no = @p_call_no)
											order by ap.event_date desc								
											)
					  and a.employee_status = 'A'
					  and a.company_id = b.company_id
					  and a.country_code = b.country_code
