/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_dynapaccomm_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_dynapaccomm_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_dynapaccomm_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_dynapaccomm_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'dynapaccomm_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'dynapaccomm_form')
	...
	if (@p_file_name like '%dynapaccomm_form%')
		begin
		...
		select @p_notification_xml = '<notification_info>'+
				...
				@i_locale_id , @i_country_code , 'CALL_EMAIL_COMM_PDF' ,@p_notification_xml, @i_user_id, @p_notification_id OUTPUT	
