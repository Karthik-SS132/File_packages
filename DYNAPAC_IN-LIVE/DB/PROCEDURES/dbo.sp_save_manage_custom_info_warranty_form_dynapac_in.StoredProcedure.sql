/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_warranty_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_warranty_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_warranty_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_warranty_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'warranty_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'warranty_form')
	...
	select part_no,part_desc,part_qnty,SUBSTRING(qrcode,CHARINDEX('service/',qrcode )+8,DATALENGTH(qrcode)) as qrcode from OPENJSON(@i_inputparam_header_xml,'$.part')
	with (
	part_no int '$.part_no_savedetails',
	part_desc nvarchar(50) '$.part_desc_savedetails',
	part_qnty int '$.part_qnty_savedetails',
	...
	select isnull(part_no,''),isnull(part_desc,''),isnull(part_qnty,'') from OPENJSON(@i_inputparam_header_xml,'$.recomm_spare') 
	with (
	...
	if (@p_file_name like '%warranty_form%')
		begin
		...
		select @p_notification_xml = '<notification_info>'+
				...
				@i_locale_id , @i_country_code , 'CALL_EMAIL_WARR_PDF' ,@p_notification_xml, @i_user_id, @p_notification_id OUTPUT	
