/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_dynapac_flup_mtng_repo_form_dynapac_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_dynapac_flup_mtng_repo_form_dynapac_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_dynapac_flup_mtng_repo_form_dynapac_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_dynapac_flup_mtng_repo_form_dynapac_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'dynapac_flup_mtng_repo_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'dynapac_flup_mtng_repo_form')
	...
	if (@p_file_name like '%dynapac_flup_mtng_repo_form%')
		begin
			...
			declare  @p_source_call_ref_no nvarchar(20), @p_frt_record_seqNo varchar(30), @p_noof_records int,
						 ...
						 @p_flup_assigned_to_emp_id varchar(15), @p_flup_problem_description nvarchar (200)
			...
			IF not exists (select 1 from call_register_actions   
				...
				begin
					...
					p_udf_analysis_code2,p_udf_analysis_code3
						from #part_temp
						where p_file_name like '%dynapac_flup_mtng_repo_form%' 
				  ...
				  set @p_frt_record_seqNo  = (select top(1) action_seqno 
				    ...
				    /* first requirement to insert into source enquiry */
					 ...
					 begin
                        ...
                        select @p_flup_logged_on_date = paramval from #input_params where paramname = 'event_date'
						select @p_flup_logged_on_hour = paramval from #input_params where paramname = 'event_hour'
						select @p_flup_logged_on_minute = paramval from #input_params where paramname = 'event_minute'
