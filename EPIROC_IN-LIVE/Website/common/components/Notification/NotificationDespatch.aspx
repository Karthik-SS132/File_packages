...
<script runat="server">
    ...
    public class Notification
    {
        ...
        public static string client_notification_smtp_sender_email_id = "";
        public static string System_support_smtp_sender_email_id = "";
        ...
        public Notification()
        {
            System.Reflection.Assembly testAssembly = System.Reflection.Assembly.LoadFile(HttpContext.Current.Server.MapPath("~/bin/appclientconfig.dll"));
            ...
            foreach (ConfigurationElement user in usersList)
            {
                ...
                client_notification_smtp_sender_email_id = user.GetType().GetProperty("client_notification_smtp_sender_email_id").GetValue(user, null).ToString();
                ...
                System_support_smtp_sender_email_id =  user.GetType().GetProperty("System_support_smtp_sender_email_id").GetValue(user, null).ToString();
    ...
    class notification_despatch
    {
                ...
                if (NotificationResponse.DocumentElement.ChildNodes[0].ChildNodes[outputparam_detail_n_header].Name == "outputparam_detail_n_header")
                {
                    ...
                    if (NotificationResponse.DocumentElement.ChildNodes[0].ChildNodes[outputparam_detail_n_header]["p_n_info_xml"].SelectSingleNode("notification_info/customer_feedback_ind") != null)
                    {
                        ...
                        if (customer_feedback_ind == "true")
                        {
                            ...
                            if (!Directory.Exists(HttpContext.Current.Server.MapPath("~/Log/Notification/TinyUrl")))
                            {
                                Directory.CreateDirectory(HttpContext.Current.Server.MapPath("~/Log/Notification/TinyUrl"));
                            }

                            File.AppendAllText(HttpContext.Current.Server.MapPath("~/Log/Notification/TinyUrl/" + Notification.Client_Id + "_" + Notification.Country_Code + "_" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt"), customer_feedback_long_url + "-->" + tinyurl + Environment.NewLine);
        ...
        public List<string> GetHtmlForEmail(string no_evnt_code, string temp_id, string no_mode, string notid, string notificationxml)
        {
            ...
            string xsl_folder_path = HttpContext.Current.Server.MapPath("~/common/components/Notification/Xsl/");
            string html_folder_path = HttpContext.Current.Server.MapPath("~/common/components/Notification/Html/");
            try
            {
        ...
        public string GetTextForPushNotification(String clientID, String countryCode, String userID, string no_evnt_code, string temp_id, string no_mode, string notid, string notificationxml, out String appID)
        {
            ...
            if (notificationText != "")
            {
                ...
                var msgToObject = JsonConvert.DeserializeObject<dynamic>(File.ReadAllText(AppDomain.CurrentDomain.BaseDirectory + "\\" + "content_store" + "\\" + clientID + "\\" + countryCode + "\\" + "msvMsg" + "\\" + "fcmtoken" + "\\" + ((String)msgObject.data.msgHeader.appID) + "\\" + "mobile" + "\\" + userID + ".txt"));
        ...
        public string SendEmail(string TO, List<string> CC, List<string> attachements, string body, string subject, string notification_mode, string notification_event_code, string temp_id, string notification_id)
        {
            ...
            try
            {
                mail.From = new MailAddress(Notification.client_notification_smtp_sender_email_id, Notification.client_notification_smtp_recipient_emailid_display_name);
                mail.To.Add(TO);
        ...
        public string SendSms(string TO, List<string> CC, string template_str, string smsgatewayurl, string notification_mode, string notification_event_code, string temp_id, string notification_id)
        {
            ...
            try
            {
                ...
                settings.EnableScript = true;
                XslCompiledTransform objXslTrans = new XslCompiledTransform();
                objXslTrans.Load(HttpContext.Current.Server.MapPath("~/common/components/Notification/smsgateway_config.xsl"), settings, new XmlUrlResolver());
                ...
                if (ResponseXml == "ND001")
                {
                    ...
                    try
                    {
                        DirectoryInfo smscountdir = new DirectoryInfo(HttpContext.Current.Server.MapPath("~/Log/Notification/smslogger"));

                        // Create the directory only if it does not already exist. 
                        if (smscountdir.Exists == false)
                            smscountdir.Create();

                        FileStream fs = new FileStream(HttpContext.Current.Server.MapPath("~/Log/Notification/smslogger/" + Notification.Client_Id + "_" + Notification.Country_Code + "_" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt"), FileMode.Append, FileAccess.Write);
        ...
        public string SendPushNotification(String clientID, String countryCode, String notificationText, string notificationMode, string notificationEventCode, string tempID, string notificationID, String appID)
        {
            ...
            try
            {
                ...
                var restAPiObject = JsonConvert.DeserializeObject<dynamic>(File.ReadAllText(HttpContext.Current.Server.MapPath("~/fmt" + "\\" + "msvMsg" + "\\" + appID + "\\" + "fcmkey_" + clientID + "_" + countryCode + ".json")));
                ...
                request.Headers.Add("authorization", "Basic " + (String)restAPiObject["mobile"].fcmKey);
                ...
                File.AppendAllText(HttpContext.Current.Server.MapPath("~/Log/Notification/push_audit_" + DateTime.Now.ToString("dd-MM-yyyy") + ".txt"), clientID + "-->" + countryCode + "-->" + notificationMode + "-->" + notificationEventCode + "-->" + tempID + "-->" + notificationID + "-->" + notificationText + "-->" + responseMessage + Environment.NewLine);
    ...
    static class Notificationloger
    {
        public static void create_Log_File(string Service_Name, string Notification_Mode, string Notification_Event_Code, string Template_Id, string Notification_Id, string Error_Description)
        {
            try
            {
                string error_log_path = HttpContext.Current.Server.MapPath("~/Log/Notification/Error");
    ...
    class ReportException
    {
        ...
        public void post(string bdy)
        {
            ...
            try
            {
                ...
                {
                    mail.From = new MailAddress(Notification.System_support_smtp_sender_email_id);
