/****** Object:  StoredProcedure [dbo].[sp_save_manage_call_register]    Script Date: 23/8/2023 10:48:00 AM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_call_register')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_call_register]
END
GO
/****** Object:  StoredProcedure [dbo].[sp_save_manage_call_register]    Script Date: 5/16/2024 10:03:39 AM ******/
SET ANSI_NULLS ON
GO
...
begin
	...
	begin
		...
		select @i_additional_description = @i_additional_description + 
			...
			if not exists ( select '*' from customer a,customer_location b 

							where a.company_id		= @i_client_id
							and   a.country_code	= @i_country_code
							and  (a.customer_id = @i_customer_id or (@i_customer_id != '' and a.customer_name like '%' + @i_customer_id + '%'))
				...
				begin
					select @i_customer_id = customer_id					
					from customer
					where company_id = @i_client_id
						and country_code = @i_country_code
						and (customer_id = @i_customer_id or (@i_customer_id != '' and customer_name like '%' + @i_customer_id + '%'))
				end
	...
	/* Acopco dealer call logging - 28.Mar.18 - Chak. To be removed after mobile screen 
		hardcoding   of call category SE is removed */

	if @i_client_id in ( 'acopco','dynapac','epiroc') and substring(@i_user_id,1,6) not in ('minacs', 'kserve','ncecnp') and @p_channel_id = 'WEB' 
	begin
	 ...
	 product_udf_char_1,product_udf_char_2,product_udf_char_3,product_udf_char_4,
	  ...
	  select @i_client_id,@i_country_code, @i_customer_id ,
		     ...
		     @p_asset_in_warranty_ind,
																				isnull( (select paramval from #input_params where paramname = 'call_register_source_code_category') ,''),
			 isnull( (select paramval from #input_params where paramname = 'call_register_source_code') ,''),
			 isnull((select CONVERT(datetimeoffset,
				 (select x.paramval from #input_params x where x.paramname = 'call_register_expected_closure_date')
				+' ' +
				 (select y.paramval from #input_params y where y.paramname = 'call_register_expected_closure_date_hour')
				 + ':' + 
				 (select z.paramval from #input_params z where z.paramname = 'call_register_expected_closure_date_minute')
				 +':00',120)), NULL),																  
  ...
  begin
	...
	begin
		...
		set call_ref_no = dbo.fn_client_specific_stamping_call_no(@i_client_id, @i_country_code, @i_call_category, @i_call_type,@p_call_no,@p_by_field_1, @p_by_field_2, @p_by_field_3,@p_by_field_4, @p_by_field_5)
		where company_id = @i_client_id
		  and country_code = @i_country_code
  ...
  /* Run Auto Assignment for declaring variable */
	...
	begin
	  ...
	  /* For Atlas Copco - Dealer creating Enquiry ticket via web application should
		...
		if @i_client_id in ( 'acopco','dynapac','epiroc') and @p_channel_id != 'mobile' and @p_organogram_level_no = 4
		...
		begin	 
			  ...
			  where company_id = @i_client_id
			    and country_code = @i_country_code
				and dealer_id = ( select b.dealer_id /* */
								  from dealer_organogram_mapping b
								  where b.company_id = @i_client_id
								    and b.country_code = @i_country_code
								    and cast(b.organogram_level_no as varchar(1)) + b.organogram_level_code = 
									( select cast(e1.organogram_level_no as varchar(1)) + e1.organogram_level_code
									  from employee e1
									  where e1.company_id = @i_client_id
									    and e1.country_code = @i_country_code
										and e1.employee_id = @p_employee_id)
									)
				and mapping_purpose_code = 'CALLAUTOASSIGNMENT'
...
/* Code Fix - Nepal Dealer */		
		if @i_client_id in ('epiroc') and @p_channel_id != 'mobile' and @p_organogram_level_no = 4
		   and @i_call_category in ('SE') and @i_user_id  in (select user_id from users where user_group_id ='SCOORD-NPL')
		begin	
			   update call_register 
			   set call_status = 'O'
			   where company_id = @i_client_id
...
begin
    ...
    if @i_client_id in ( 'acopco','dynapac','epiroc')
    begin
		
  
		/* For users other than call center users */
		if substring(@i_user_id,1,6) not in  ('minacs','kserve','ncecnp')
		begin
	  ...
	  end /* for users other than minacs, kserve*/
...
/* 1.May.2018 - Removed updating organogram code as client sending this code is buggy. 
   Organogram should not be changed during update. Only action verbs can modify organogram
   */	  
 update call_register
