   ...
   begin
	...
	begin
		SELECT '' as value_list,
			...
			'' as o_value_field_2,
			'' as o_value_field_3,
			c.title + ' ' + c.first_name + ' ' + ISNULL(c.middle_name,'') + ' ' + c.last_name + ' (' +ISNULL(c.udf_char_1,'')+ ')' as o_description_field_1,
			'' as o_description_field_2,
			'' as o_description_field_3
	...
	begin
		...
		where company_id = @i_client_id 
			...
			and service_visit_slno > 
				( select MAX(as1.service_visit_slno)
	...
	else if (@i_lov_code = 'EQUIPMENTCATEGORY_LIST')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.code + '",' +
