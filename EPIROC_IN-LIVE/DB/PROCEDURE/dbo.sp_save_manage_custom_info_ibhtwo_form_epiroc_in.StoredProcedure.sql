/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_ibhtwo_form_epiroc_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_ibhtwo_form_epiroc_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_ibhtwo_form_epiroc_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_ibhtwo_form_epiroc_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'ibhtwo_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'IBH 10-II-COUPON')
