...
BEGIN
	...
	declare @p_request_category varchar(10),
		...
		@p_call_created_by_user_id nvarchar(12),
	...
	begin
		...
		where a.company_id = @i_client_id
			...
			and b.country_code = a.country_code
			and b.employee_id = a.employee_id
			and a.user_group_id = 'SCOORD-OEM'
	...
	begin
		...
		begin
			...
			begin
				...
				if @p_error_no != ''
				begin
					set @i_error_msg = @p_call_created_by_employee_id+ '{"code":"failed_creating_enquiry"}'
					return
				end	
			...
			begin
				...
				begin
					...
					where company_id = @i_client_id
					 ...
					 select @p_call_created_by_user_id = user_id 
					   from users 
					   where company_id=@i_client_id
						 and country_code=@i_country_code
					     and employee_id in (select employee_id 
											 ...
											 where company_id=@i_client_id
											  ...
											  and employee_id=@p_call_created_by_employee_id )
				end
				else
				begin
					select top(1) @p_call_created_by_employee_id = a.employee_id
					from users a, employee b
					where a.company_id = @i_client_id
						and a.country_code = @i_country_code
						and b.company_id = a.company_id
				...
				execute sp_save_manage_call_register @i_client_id, @i_country_code, @i_session_id, @p_call_created_by_user_id, @i_locale_id, 
						@p_customer_id, @p_asset_id, '', @p_equipment_id, @p_problem_description, 'MED', @p_call_created_by_user_id,
						@p_logged_on_date, @p_logged_on_hour, @p_logged_on_minute, @p_customer_location_code, @p_organogram_level_no,
						@p_organogram_level_code, @p_call_category,	@p_call_type,'', @p_company_location_code,
						@p_cust_contact_person_1, @p_cust_contact_person_1_mobile_no, @p_cust_contact_person_1_email_id, 'NB',
						0,0,0,0,'INR','','', '','', @p_call_inputparam,
				...
				if @p_error_no != ''
				begin
					set @i_error_msg = '{"code":"failed_creating_service"}'
					return
				end	
		...
		begin
			...
			select @p_logged_on_minute = substring(convert(varchar(10),SYSDATETIMEOFFSET(),108),4,2)
			select @p_problem_description = isnull(json_value(@i_inputparam_header_xml, '$.msgBody.custom.problem_desc'), '')
			set @p_call_inputparam = '<inputparam><
			>'+'CQM'+'</call_register_source_code_category><call_register_source_code>'+@p_transaction_ref_no+'</call_register_source_code></inputparam>'
