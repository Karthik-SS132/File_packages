/* Mservice Common Scripts - Version 1.0.06_acopco - 21/09/2021 */
...
var mserviceUtilities = {
	...
	getAllowedFileSize : function () {
		if (typeof(mserviceUtilities.variable.userAttachmentsAllowedFileSize) == "undefined") {
			mserviceUtilities.variable.userAttachmentsAllowedFileSize = ConvertXMLStringToJSONArray(executeService_retrieve_listof_values_for_searchcondition({
						p_inputparam_xml : "<inputparam><lov_code_type>FILESIZEALLOWED_CONFIGURATION</lov_code_type></inputparam>"
					}));
		};
		return mserviceUtilities.variable.userAttachmentsAllowedFileSize;
	},
	...
	getAllowedWorkflowEventVerbs: function (callObject) {
		...
		allowedWorkflowArray = $.grep(mserviceUtilities.variable.WorkflowObject[callObject.transactionType], function (element, index) {
				return ((element.request_category == callObject.requestCategory && element.request_type == callObject.requestType && element.from_workflow_stage == callObject.fromStage && element.from_status == callObject.fromStatus) || (element.request_category == callObject.requestCategory && element.request_type == "ALL" && element.from_workflow_stage == callObject.fromStage && element.from_status == callObject.fromStatus) || (element.request_category == "ALL" && element.request_type == "ALL" && element.from_workflow_stage == callObject.fromStage && element.from_status == callObject.fromStatus));
		});
	...
	getNextWorkflowEventVerbs: function (callObject) {
		var allowedWorkflowArray;
			...
			allowedWorkflowArray = $.grep(mserviceUtilities.variable.WorkflowObject[callObject.transactionType], function (element, index) {
					return ((element.event_verb == callObject.eventVerb && element.request_category == callObject.requestCategory && element.request_type == callObject.requestType && element.from_workflow_stage == callObject.fromStage && element.from_status == callObject.fromStatus) || ( element.event_verb == callObject.eventVerb && element.request_category == callObject.requestCategory && element.request_type == "ALL" && element.from_workflow_stage == callObject.fromStage && element.from_status == callObject.fromStatus) || (element.event_verb == callObject.eventVerb && element.request_category == "ALL" && element.request_type == "ALL" && element.from_workflow_stage == callObject.fromStage && element.from_status == callObject.fromStatus));
			});
	...
	getInputparamXML : function (helperObject) {
		...
		for (fieldCounter = 0; fieldCounter < inputparamFields.length; fieldCounter++) {
			...
			if ($("#" + fieldID).attr("data-widget-type") == "w_attachment") {
				1==1;
			} else if ($("#" + fieldID).attr("data-widget-type") == "w_datepicker") {
				if ($("#" + fieldID).getVal() != "") {
		...
		if (textstatus == "success") {
			...
			if (api) {
				if (responseData.ApplicationException == null) {
					if ((outputPath != false) && (responseData[outputPath] == null) && noRecordAlert == true) {
