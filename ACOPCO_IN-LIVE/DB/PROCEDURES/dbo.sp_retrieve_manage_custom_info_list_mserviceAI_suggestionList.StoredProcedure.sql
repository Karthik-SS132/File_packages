...
BEGIN
	...
	else if (@p_suggestion_list_type = 'CQMEQUIPMODELLT')
	begin

		select  '' as custom_info_list,
		'{' +
			'"title":"' + dbo.code_description(company_id, country_code, @i_locale_id,'CQMEQUIPMODELLT', code) + '",' +
			'"value":"' + '@CQMEQUIPMODELLT' + '",' +
			'"custom":' + 
			'{' + 
				'"cqmEquipModelLt":"' + code + '"' +
			'}' +
		'}' as o_custom_info_json
		from code_table
		where company_id = @i_client_id
			and country_code = @i_country_code
			and code_type = 'CQMEQUIPMODELLT'
