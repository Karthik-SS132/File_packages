...
GO
/****** Object:  StoredProcedure [dbo].[sp_save_manage_asset_master]    Script Date: 11/6/2023 3:10:18 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	declare @p_effective_from_date datetimeoffset(7), @p_effective_to_date datetimeoffset(7),
			...
			@p_asset_category nvarchar(15), @p_asset_type nvarchar(15), 
			@p_company_location_code nvarchar(8), @p_customer_location_code nvarchar(10),
			@p_child_asset_id nvarchar(30), @p_child_equipment_oem nvarchar(60), @p_child_equipment_category nvarchar(15),
			@p_child_equipment_type nvarchar(15), @p_child_equipment_id nvarchar (30)
 ...
 (paramname varchar(50) not null,
  ...
  FROM @p_inputparam_xml.nodes('/inputparam/*') AS Tbl(nodes)
	...
	set @p_asset_category = (select paramval from #input_params where paramname = 'asset_category')
	set @p_asset_type = (select paramval from #input_params where paramname = 'asset_type')	
	set @p_company_location_code = (select paramval from #input_params where paramname = 'company_location')
	set @p_customer_location_code = (select paramval from #input_params where paramname = 'customer_location')
	set @p_child_asset_id = (select paramval from #input_params where paramname = 'child_asset_id')
	set @p_child_equipment_oem = (select paramval from #input_params where paramname = 'child_equipment_oem')
	set @p_child_equipment_category = (select paramval from #input_params where paramname = 'child_equipment_category')
	set @p_child_equipment_type = (select paramval from #input_params where paramname = 'child_equipment_type')
	set @p_child_equipment_id = (select paramval from #input_params where paramname = 'child_equipment_id')
 ...
 begin
    ...
    insert asset_master
		(
		  ...
		  installation_date, asset_category, asset_type,
			   ...
			   (case @i_installation_date 
				...
				case (select applicable from #applicable_custom_fields
				 ...
				 case 
				  ...
				  isnull(@p_customer_location_code,''),
				  isnull(@p_company_location_code,''),
				  @i_user_id
    ...
    begin
          ...
          /* Pick default currency code from company_location for the user's location
			...
			select @i_client_id, @i_country_code, @i_asset_id,
				   ...
				   (case contract_type
					...
					when 'OW' then 'OUTOFWARRANTY'
					when 'AMC' then 'Annual Maintenance Contract'
				    end), 
				   (case contract_type
				    when 'IW' then 'Initial Warranty'
				    when 'EW' then 'Extended Warranty'
					when 'OW' then 'Out of Warranty'
					when 'AMC' then 'Annual Maintenance Contract'
				    end),
				   contract_duration, 
			...
			begin
			  ...
			  if exists ( select 1  from equipment_service_schedule_params
				...
				if @@ROWCOUNT = 0
				begin
					raiserror('Failed to create service schedule parameters',15,1)
				...
				begin
					...
					if not exists ( select 1 from asset_master_boa 
								where company_id = @i_client_id
								  and country_code = @i_country_code
								  and child_asset_id = @i_asset_id
								)
					begin
				  ...
				  end /* Only for main machines and not for bill of assembly components */

				end /* 				if exists ( select 1 from asset_service_contract */
	  ...
	  /* sub assembly entry - start */
	  
											
								  
									  
		if @p_child_asset_id = @i_asset_id
	
		begin

			raiserror('Machine and Sub Assembly Id cannot be the same',15,1)
			set @errorNo = 'E_UP_023'
			return
		...
		if isnull(@p_child_asset_id,'') <> ''
		begin
			if not exists (select 1 from asset_master 
				where company_id = @i_client_id
					and country_code = @i_country_code
					and asset_id = @p_child_asset_id)
			begin
				...
				(
					...
					creation_date, created_by_emp_id,
					asset_location_country_code,asset_location_state_code,asset_location_district_code,																					  
					customer_location_code, company_location_code, parent_asset_id, last_update_id
				)
				select @i_client_id, @i_country_code, @p_child_asset_id, 
					@p_child_equipment_id,
					@i_customer_id, @i_asset_location_code, @i_locator_layout,
					(case @i_installation_date 
					when '' then null
					else CONVERT(datetimeoffset,@i_installation_date+' 00:00:00',120)
					...
					isnull((select paramval from #input_params where paramname = 'udf_asset_location_state_code') ,''),
					isnull((select paramval from #input_params where paramname = 'udf_asset_location_district_code') ,''),
					isnull(@p_customer_location_code,''), isnull(@p_company_location_code,''), @i_asset_id, @i_user_id
				...
				if exists ( select 1 from equipment_service_contract_template
						where company_id = @i_client_id
						and country_code = @i_country_code
						and equipment_id = @p_child_equipment_id
						and contract_type = 'IW')
				begin
					...
					begin
						...
						begin
							...
							if exists ( select 1  from equipment_service_schedule_params
      						...
      						select  company_id, country_code, @p_child_asset_id, 
      							(case contract_type
								when 'IW' then 'INITIALWARRANTY'
								when 'EW' then 'Extended Warranty'
								when 'OW' then 'Out of Warranty'
							...
							begin
								...
								execute sp_create_asset_service_schedule_basedon_contract @i_client_id, @i_country_code,
									@i_user_id , @p_child_equipment_id, @p_child_asset_id, 'INITIALWARRANTY', 'NB', 
								@p_default_currency_code, 0, 0, 0, 0,	@o_update_status OUTPUT, @errorNo OUTPUT
								...
								begin
									...
									/* Recalculate the next visit dates */
									execute sp_update_asset_service_schedule_cascade_dates @i_client_id, @i_country_code, @i_user_id , 
											@p_child_asset_id, 'INITIALWARRANTY', 0,	@o_update_status OUTPUT, @errorNo OUTPUT
		...
		begin
			...
			insert asset_master_boa
			(
				company_id, country_code, asset_id, child_asset_id,
				child_equipment_oem, child_equipment_category, child_equipment_type, 
				child_equipment_id, child_asset_status,
				installation_date, creation_date, created_by_emp_id, last_update_id
			)
			select @i_client_id, @i_country_code, @i_asset_id, @p_child_asset_id,
				@p_child_equipment_oem,
				@p_child_equipment_category,
				@p_child_equipment_type,
				@p_child_equipment_id, @i_asset_status,
				(case @i_installation_date 
					when '' then null
					else CONVERT(datetimeoffset,@i_installation_date+' 00:00:00',120)
				end),
				SYSDATETIMEOFFSET(), @i_user_id, @i_user_id
 ...
 begin
        ...
        begin     
				...
				set equipment_id = @i_equipment_id,
					...
					customer_location_code = isnull(@p_customer_location_code,''),
					company_location_code = isnull(@p_company_location_code,''),							  
					last_update_id=@i_user_id
			where company_id = @i_client_id
			  and country_code = @i_country_code
			  and asset_id = @i_asset_id
			...
			begin
				...
				if exists ( select 1 from call_register
							where company_id = @i_client_id
							  ...
							  and call_type = 'COMM')
				begin
					raiserror('Commissioning date cannot be changed as there is a commissioning job in the system', 15,1)
					set @errorNo = 'E_UP_023'
					return
				...
				begin
				  ...
				  /* Pick default currency code from company_location for the user's location
					...
					select @i_client_id, @i_country_code, @i_asset_id,
						   contract_type,  
						   'INITIALWARRANTY',
						   dbo.code_description(@i_client_id, @i_country_code, @i_locale_id, 'ASSETCONTRACTTYPE', contract_type),
						   contract_duration, 
						   contract_duration_uom,
						   @p_effective_from_date,
						   @p_effective_to_date,
			...
			/* sub assembly update - start */
			if ISNULL(@p_child_asset_id,'') <> ''
			begin
				if exists (select 1 from asset_master
						where company_id = @i_client_id
						  and country_code = @i_country_code
						  and parent_asset_id = @i_asset_id)
				begin

					select top(1) @p_child_asset_id = asset_id, @p_child_equipment_id = equipment_id from asset_master
							where company_id = @i_client_id
							  and country_code = @i_country_code
							  and parent_asset_id = @i_asset_id
					...
					update asset_master_boa
					set child_asset_status = @i_asset_status,
						installation_date = 
							( case @i_installation_date 
							  when '' then null
							  else CONVERT(datetimeoffset,@i_installation_date+' 00:00:00',120)
					...
					begin
						...
						create table #child_asset_existing_service_schedule
						(
							service_visit_slno tinyint null,
							call_ref_jo_no nvarchar(20) null,
							call_jo_ind varchar(1) null,
						...
						begin
							...
							insert #child_asset_existing_service_Schedule
							(
								service_visit_slno, call_ref_jo_no, call_jo_ind, service_visit_status
							)
							select service_visit_slno, call_ref_jo_no, call_jo_ind, service_visit_status
						...
						/* Set the visit schedule service due dates */
							...
							update asset_service_schedule
								set service_visit_status = b.service_visit_status,
									call_jo_ind = b.call_jo_ind,
									call_ref_jo_no = b.call_ref_jo_no
							from #child_asset_existing_service_schedule b
							where asset_service_schedule.company_id = @i_client_id
							  and asset_service_schedule.country_code = @i_country_code
							  and asset_service_schedule.asset_id = @p_child_asset_id
							  and asset_service_schedule.contract_doc_no = 'INITIALWARRANTY'
							  and asset_service_schedule.service_visit_slno > 0
							  and asset_service_schedule.service_visit_slno = b.service_visit_slno
