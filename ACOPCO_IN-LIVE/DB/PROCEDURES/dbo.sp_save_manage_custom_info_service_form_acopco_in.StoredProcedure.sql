/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_service_form_acopco_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_service_form_acopco_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_service_form_acopco_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_service_form_acopco_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'service_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'service_form')
	...
	/* Array of object savefield values handed */
	...
	select part_no,part_desc,part_qnty,SUBSTRING(qrcode,CHARINDEX('service/',qrcode )+8,DATALENGTH(qrcode)) as qrcode from OPENJSON(@i_inputparam_header_xml,'$.repair_spare')
	with (
	part_no int '$.part_1_savedetails',
	part_desc nvarchar(50) '$.desc_1_savedetails',
	part_qnty int '$.qty_1_savedetails',
	qrcode nvarchar(500) '$.qrcode_part'
	)
	...
	select part_no,part_desc,part_qnty from OPENJSON(@i_inputparam_header_xml,'$.recomm_spare') 
	with (
	part_no int '$.rs_part_1_savedetails',
	part_desc nvarchar(50) '$.rs_desc_1_savedetails',
	part_qnty int '$.rs_qty_1_savedetails'
	)
	
	
	
	if (@p_file_name like '%service_form%')
		begin
			...
			select @p_notification_xml = '<notification_info>'+
					...
					if @p_file_name like '%service_form%'
					    ...
					    begin
						    ...
						    @i_locale_id , @i_country_code , 'CALL_EMAIL_SR_PDF' ,@p_notification_xml, @i_user_id, @p_notification_id OUTPUT
						end
			...
			select @i_client_id,@i_country_code,@p_transaction_ref_no,'PARTRECOMM','PARTRECOMM',part_no,'NA',part_desc,part_qnty,'Nos',qrcode
			from #partdetails where part_desc !=''
