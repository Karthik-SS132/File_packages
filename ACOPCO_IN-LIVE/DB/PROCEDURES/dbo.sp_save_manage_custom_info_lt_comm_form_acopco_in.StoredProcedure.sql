/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_lt_comm_form_acopco_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_lt_comm_form_acopco_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_lt_comm_form_acopco_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_lt_comm_form_acopco_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'lt_comm_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'lt_comm_form')
	...
	if (@p_file_name like '%lt_comm_form%')
		begin
