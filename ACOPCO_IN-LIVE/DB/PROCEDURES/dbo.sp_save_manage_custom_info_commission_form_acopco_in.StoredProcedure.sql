/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_commission_form_acopco_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_commission_form_acopco_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_commission_form_acopco_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_commission_form_acopco_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'commission_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'commission_form')
	...
	if (@p_file_name like '%commission_form%')
		begin
