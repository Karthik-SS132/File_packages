DROP PROCEDURE IF EXISTS[dbo].[sp_save_manage_custom_info_call_status_change_vi_acopco]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_call_status_change_vi_acopco] 
...
BEGIN
	...
	if (@p_eventverb_id = 'MEETINGSTART')
	begin
	...
	else if (@p_eventverb_id = 'MEETINGFINISH')
	begin
	...
	if @p_eventverb_id = 'ASSIGN' or @p_eventverb_id = 'REASSIGN'
		begin
		declare  
		@p_dealer_id nvarchar (13),
		@p_call_mapped_to_emp_id  nvarchar (20),
		@p_call_mapped_to_func_role nvarchar (20),
		@p_equipment_category nvarchar (20),
		@p_equipment_type nvarchar (20)
			...
			begin
				...
				if @@ROWCOUNT = 0
				begin
					set @i_error_msg = '{"code":"Failed Updating Call Assignment - Resetting Primary Resource"}'
					return
				end		
			...
			if not exists ( select 1 from call_assignment
						where company_id = @i_client_id
						  and country_code = @i_country_code
						  and call_ref_no = @p_transaction_ref_no
						  and resource_emp_id = @p_to_employee_id_string
						  and primary_resource_ind = 1)
			begin
				...
				select @i_client_id, @i_country_code, @p_transaction_ref_no,
						@p_to_employee_id_string, 1, @p_event_date, @i_user_id

				if @@ROWCOUNT = 0
				begin
					set @i_error_msg = '{"code":"Failed Insert into Call Assignment"}'
					return
				end		
				...
				where company_id = @i_client_id
				  ...
				  and cast(organogram_level_no as varchar(1)) + organogram_level_code
					 = ( select cast(organogram_level_no as varchar(1)) + organogram_level_code
						 ...
						 where company_id = @i_client_id 
						   and country_code = @i_country_code
						   and employee_id = @p_to_employee_id_string /* To be fixed when multi engineer assignment */
						)
			...
			begin
				...
				if @@ROWCOUNT = 0
				begin
					set @i_error_msg = '{"code":"Failed Updating Call Assignment"}'
					return
				end		
		  ...
		  /* Pending - In case of reassign, check for plan start date to be after hold date */
			...
			/*if DATEDIFF(MINUTE,(CONVERT(datetimeoffset,
								...
								(CONVERT(datetimeoffset,
									 ...
									 +':00',120))) < 0 
			begin
				set @i_error_msg = '{"code":"Plan finish date cannot be earlier than Plan start date"}'
				return
			end
			*/
			...
			set call_wf_stage_no = @p_to_wf_stage_no,
				...
				sch_start_date =GETDATE(),
				sch_finish_date = GETDATE(),				
				last_update_id = @i_user_id
			where company_id = @i_client_id
			  and country_code = @i_country_code
			  and call_ref_no = @p_transaction_ref_no
			...
			if @@ROWCOUNT = 0
			begin
				set @i_error_msg = '{"code":"Failed updating Call Register"}'
				return
			end
			...
			begin
				...
				select @i_client_id, @i_country_code, @p_transaction_ref_no, resource_emp_id,
				...
				if @@ROWCOUNT = 0
				begin
					set @i_error_msg = '{"code":"Failed Updating the call"}'
					return
				end		  
			end
	...
	else if (@p_eventverb_id in ('MEETINGFINISH','COMPLETE','CLOSE','HOLD', 'ASSIGN', 'REASSIGN','SAVEFORASSIGN','PROGRESSUPDATE','REJECTCALL'))
	begin
		set @p_allow_new_txn = 1
	end
	...
	begin
		...
		declare @p_notification_event_code_1 nvarchar(60),
			...
			@p_file_name nvarchar(500),
			@p_assign_to_emp_id nvarchar(12),
			@p_attachments_string nvarchar(max)

		 select top(1) @p_file_name = attachment_file_name 
			...
			where b1.company_id = @i_client_id
			  ...
			  and b1. assigned_on_date = 
					( select MAX(b2.assigned_on_date )
						from call_assignment b2
						where b1.company_id = b2.company_id
						and b2.call_ref_no = b1.call_ref_no)


		select @p_file_path = 'call_attachments/'+@p_transaction_ref_no+'/'+@p_file_name

		select @p_attachments_string = '<attachment>'+@p_file_path+'</attachment>'
		...
		begin
			...
			begin
				...
				select @p_notification_xml = '<notification_info>'+
					...
					'<call_act_finish_date>'+CONVERT(varchar(17), a.act_finish_date,113)+'</call_act_finish_date>'+
					...
					'<customer_feedback_ind>true</customer_feedback_ind>' +
					'<customer_feedback_long_url>' + 'feedback/feedback_index.html?call_ref_no=' + @p_transaction_ref_no + '&call_type=' + a.call_type + '</customer_feedback_long_url>' +
					'<customer_feedback_short_url></customer_feedback_short_url>' +
					'</notification_info>'		
