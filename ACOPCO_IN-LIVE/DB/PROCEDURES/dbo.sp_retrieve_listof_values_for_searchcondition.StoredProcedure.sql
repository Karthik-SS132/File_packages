...
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_retrieve_listof_values_for_searchcondition')
BEGIN
...
BEGIN
	...
	DECLARE @p_SQLString nvarchar(1000) , @p_ParmDefinition nvarchar(1000),
	...
	begin
		...
		EXECUTE sp_executesql @p_SQLString, @p_ParmDefinition,
							  @p_session_id = @i_session_id, 
								...
								@p_search_field_6 = @p_search_field_6,
								@p_search_field_7 = @p_search_field_7,
								@p_search_field_8 = @p_search_field_8,
	...
	begin
		...
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.asset_id like '%' + @p_search_field_1 + '%'
			and a.asset_category like '%' + isnull(@p_search_field_2,'') + '%'
			and a.asset_type like '%' + isnull(@p_search_field_3,'') + '%'
			and a.equipment_id like '%' + isnull(@p_search_field_4,'') + '%'
			and a.customer_id like '%' + isnull(@p_search_field_5,'') + '%'		   
			and b.company_id = a.company_id
			and b.country_code = a.country_code
			and b.equipment_id = a.equipment_id
		return
...
if (@p_lov_code_type = 'CHARGETYPE_LIST_LINKED')
	begin
		...
		begin
			...
			where company_id = @i_client_id
			and  country_code = @i_country_code
			and  applicable_at_lineitem_level_ind in( 1,0)
			and  system_or_user_selectable_ind = 'U'
		end
		else 
		begin
			select distinct '' as value_list,
			...
			where company_id = @i_client_id
			and  country_code = @i_country_code
			and  applicable_at_lineitem_level_ind = 1
			and  system_or_user_selectable_ind = 'U'
		end
		return
	end
	...
	begin
		...
		begin
			...
			where company_id = @i_client_id
			and  country_code = @i_country_code
			and  additional_charges_ind = @p_search_field_2
			and  applicable_at_lineitem_level_ind in( 1,0)
			and  system_or_user_selectable_ind = 'U'
	...
	if (@p_lov_code_type = 'DBDEALER_LIST')
	begin

		if @p_search_field_1 = '%' set @p_search_field_1 = null

		select '' as value_list,
			'{' +
				'"code":"' + a.dealer_id + '",' +
				'"description":"' + a.dealer_name_short + '"' +
			'}' as o_value_xml
		from dealer_master a, dealer_company_location_mapping b
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.company_id = b.company_id
			and a.country_code = b.country_code
			and a.dealer_id = b.dealer_id
		    and b.company_location_code = isnull(@p_search_field_1, b.company_location_code)
...
if (@p_lov_code_type = 'COMPANY_FEATURE_INFO')
	...
	begin
		select '' as value_list,
		   '{' + 
			   '"id":"'+equipment_id+'",'+
			   '"desc":"'+description+'",'+
				'"oem":"'+isnull(equipment_oem,'')+'",'+
			   '"oem_desc":"'+ isnull(dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'EQUIPOEM', equipment_oem),'') +'",'+
			   '"category":"'+isnull(equipment_category,'')+'",'+
			   '"category_desc":"'+ isnull(dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'EQUIPCATG', equipment_category),'') +'",'+
			   '"type":"'+isnull(equipment_type,'')+'",'+
			   '"type_desc":"'+ isnull(dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'EQUIPTYPE', equipment_type),'') +'",'+			 
			   '"org_no":"'+convert(varchar(1), servicing_org_level_no)+'",'+
			   '"org_code":"'+servicing_org_level_code+'"'+
		   '}' as o_value_xml
		from equipment
	...
	begin
		...
		if (@p_lov_code_type = 'EQUIPMENTCATEGORY_LIST_LINKED')
	begin
		select '' as value_list,
			'{' +
				'"code":"ZZZ",' +
				'"description":"No Product group"' +
			'}' as o_value_xml
		union all
		select '' as value_list,
			'{' +
		...
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.link_type = 'EM'
			and a.category_code_type = 'EQUIPOEM'
			and a.category_code_value like @p_search_field_1
		return
	end
	if (@p_lov_code_type = 'DBEQUIPMENTCATEGORY_LIST_LINKED')
	begin

		if @p_search_field_1 = '%' select @p_search_field_1 = null
		...
		where a.company_id = @i_client_id
			...
			and a.link_type = 'EM'
			and a.category_code_type = 'EQUIPOEM'
			and a.category_code_value = isnull( @p_search_field_1, a.category_code_value)
		return
	end
	...
	if (@p_lov_code_type = 'DBEQUIPMENTTYPE_LIST_LINKED')
	begin
		if @p_search_field_1 = '%' set @p_search_field_1 = null
...
if (@p_lov_code_type = 'WAREHOUSEMAPLOC_LIST')
	...
	begin
		select '' as value_list,
			'{' +
				...
				'"customer_address_line_1":"' + a.address_line_1 +'"'+
			'}' as o_value_xml
		from customer a
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
	...
	if (@p_lov_code_type = 'SUPPORT_DASHBOARD_INDIVIDUAL')
	begin
		select '' as value_list,
			'{' + 
				'"msgID":1,'+
				'"msg":"Punch In @ ' +  
					isnull(right('0' + cast(start_hour as varchar(2)), 2) + ':' + 
					right('0' + cast(start_minute as varchar(2)), 2),'') + 
					' Punch Out @ '+
					isnull(right('0' + cast(finish_hour as varchar(2)), 2) + ':' + 
					right('0' + cast(finish_minute as varchar(2)), 2),'') +
					' - '+ cast(isnull(time_spent_in_mins,0)/60 as varchar(5))+' hrs'+'"'+		
			'}' as o_value_xml
		from timecard
		where company_id = @i_client_id
			and country_code = @i_country_code
			and employee_id = @p_search_field_1
			and DATEDIFF(dd, work_date, sysdatetimeoffset()) = 0
		union
		select '' as value_list,
			'{' + 
				'"msgID":'+CAST(event_id as varchar(9))+','+
				'"msg":"@ '+substring(convert(varchar(30), event_date, 121),12,5) +' - '+ 
							eventverb_id + ' on ' + call_ref_no
					 + '"' +
			'}' as o_value_xml
		from call_status_event_log
		where company_id = @i_client_id
			and country_code = @i_country_code
			and by_employee_id = @p_search_field_1
			and DATEDIFF(dd, event_date, sysdatetimeoffset()) = 0
	...
	if (@p_lov_code_type = 'DEVICE_PROFILE_UPDATE')
	begin
		
		declare @p_d_model nvarchar(50), @p_d_platform nvarchar(50), @p_d_manufacturer nvarchar(50),
			@p_d_version nvarchar(50), @p_app_version nvarchar(50), @p_device_id varchar(100),
			@p_app_id varchar(50), @p_channel_id varchar(10)
		
					
		select @p_d_model = paramval from #input_params where paramname = 'model'
		select @p_d_platform = paramval from #input_params where paramname = 'platform'
		select @p_d_manufacturer = paramval from #input_params where paramname = 'manufacturer'
		select @p_d_version = paramval from #input_params where paramname = 'device_version'
		select @p_app_version = paramval from #input_params where paramname = 'app_version'
		select @p_device_id = paramval from #input_params where paramname = 'device_id'
		select @p_app_id = paramval from #input_params where paramname = 'app_id'
		select @p_channel_id = paramval from #input_params where paramname = 'channel_id'
		...
		begin
			...
			insert device_register (
				company_id, country_code, device_id, app_id,
				app_version, user_id, employee_id, active_ind,
				channel_id, device_platform, model, manufacturer,
				device_version, last_update_id
			)
			select @i_client_id, @i_country_code, @p_device_id, @p_app_id,
				@p_app_version, @i_user_id, (
					select employee_id from users 
					where company_id = @i_client_id 
						and country_code = @i_country_code
						and user_id = @i_user_id
				), 1, @p_channel_id, @p_d_platform, @p_d_model, @p_d_manufacturer,
				@p_d_version, @i_user_id
		...
		begin
			...
			set app_version = @p_app_version,
				model = @p_d_model,
				device_platform = @p_d_platform,
				manufacturer = @p_d_manufacturer,
				device_version = @p_d_version,
	...
	*/
	...
	if (@p_lov_code_type = 'EQUIPMENTOEM_LIST')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.code + '",' +
	...
	if (@p_lov_code_type = 'DBEQUIPMENTOEM_LIST')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.code + '",' +
	...
	if (@p_lov_code_type = 'CUSTOMER_EQUIPMENTOEM_LIST')
	begin
		select distinct '' as value_list,
			'{' +
				'"code":"' + a.code + '",' +
	...
	if (@p_lov_code_type = 'DBCOMPANYLOCATION_LIST')
	begin
	...
	if (@p_lov_code_type = 'DBSERVICECONTRACT_LIST')
	begin

		select '' as value_list,
			'{' +
				'"code":"' + code+ '",' +
				'"description":"' + dbo.code_description(@i_client_id, @i_country_code, @i_locale_id, code_type, code )+ '"' +
			'}' as o_value_xml
		from code_table
		where company_id = @i_client_id
			and country_code = @i_country_code
			and code_type = 'ASSETCONTRACTTYPE'
		return
	end
	
	if (@p_lov_code_type = 'ASSET_CATG_BASED_EQUIP_OEM')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.type_code_value + '",' +
				'"description":"' + dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'EQUIPOEM',a.type_code_value)
					+ '"' +
			'}' as o_value_xml
		from category_type_link a
		where a.company_id = @i_client_id
	...
	if (@p_lov_code_type = 'ASSET_CATG_BASED_EQUIP_CATG')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.type_code_value + '",' +
				'"description":"' + dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'EQUIPCATG',a.type_code_value)
					+ '"' +
			'}' as o_value_xml
		from category_type_link a
		where a.company_id = @i_client_id
	...
	if (@p_lov_code_type = 'EQUIP_CATG_BASED_EQUIP_TYPE')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.type_code_value + '",' +
	...
	if (@p_lov_code_type = 'EQUIP_CATG_AND_TYPE_BASED_EQUIPMENT')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.equipment_id + '",' +
		...
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.equipment_id like '%' + @p_search_field_1 + '%'
			and a.equipment_category = @p_search_field_2
			and a.equipment_type = @p_search_field_3
		return
	end

	if (@p_lov_code_type = 'ASSETCATG_LIST')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.code + '",' +
				'"description":"' + dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'ASSETCATG',a.code)
					+ '"' +
			'}' as o_value_xml
		from code_table a
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.code_type = 'ASSETCATG'
		return
	end	

	if (@p_lov_code_type = 'ASSETTYPE_LIST')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.code + '",' +
				'"description":"' + dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'ASSETTYPE',a.code)
					+ '"' +
			'}' as o_value_xml
		from code_table a
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.code_type = 'ASSETTYPE'
		return
	end	

	if (@p_lov_code_type = 'ASSETTYPE_LIST_LINKED')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.type_code_value + '",' +
				'"description":"' + dbo.code_description(@i_client_id,@i_country_code,@i_locale_id,'ASSETTYPE',a.type_code_value)
					+ '"' +
			'}' as o_value_xml
		from category_type_link a
		where a.company_id = @i_client_id
	...
	if (@p_lov_code_type = 'GET_ASSET_CONTRACT_STATUS')
	begin
		if exists (select 1 from call_register a
			where a.company_id = @i_client_id
				and a.country_code = @i_country_code
				and a.asset_id = @p_search_field_1
				and a.call_ref_no = @p_search_field_2)
		begin
			if exists (select 1 from call_register a
			where a.company_id = @i_client_id
				and a.country_code = @i_country_code
				and a.asset_id = @p_search_field_1
				and a.call_ref_no = @p_search_field_2
				and a.call_type = 'COMM')
			begin
				select '' as value_list,
				   '{' + 
					   '"status":"'+'NC'+'",'+
					   '"status_desc":"'+'Not Commissioned'+'"'+
				   '}' as o_value_xml
			end
			else if exists (select 1 from asset_service_contract a
				where a.company_id = @i_client_id
					and a.country_code = @i_country_code
					and a.asset_id = @p_search_field_1
					and isnull((select  cr.act_start_date from call_register cr
						where cr.company_id = @i_client_id
							and cr.country_code = @i_country_code
							and cr.asset_id = @p_search_field_1
							and cr.call_ref_no = @p_search_field_2),SYSDATETIMEOFFSET()) between a.effective_from_date and a.effective_to_date)
			begin
				select top(1) '' as value_list,
				   '{' + 
					   '"status":"'+a.contract_type+'",'+
					   '"status_desc":"'+a.description+'"'+
				   '}' as o_value_xml
				from asset_service_contract a
				where a.company_id = @i_client_id
					and a.country_code = @i_country_code
					and a.asset_id = @p_search_field_1
					and isnull((select  cr.act_start_date from call_register cr
			...
			begin
				select '' as value_list,
				   '{' + 
					   '"status":"'+'NA'+'",'+
					   '"status_desc":"'+'Not Applicable'+'"'+
				   '}' as o_value_xml
			end
		end
		else
		begin
			if exists (select 1 from asset_master a
			where a.company_id = @i_client_id
				and a.country_code = @i_country_code
				and a.asset_id = @p_search_field_1
				and a.installation_date is null)
			begin
				select '' as value_list,
				   '{' + 
					   '"status":"'+'NC'+'",'+
				...
				where a.company_id = @i_client_id
					and a.country_code = @i_country_code
					and a.asset_id = @p_search_field_1
					and SYSDATETIMEOFFSET() between a.effective_from_date and a.effective_to_date)
			begin
				select top(1) '' as value_list,
				   '{' + 
					   '"status":"'+a.contract_type+'",'+
	...
	if (@p_lov_code_type = 'EQUIPMENT_LIST_LINKED')
	begin
		select '' as value_list,
			'{' +
				'"code":"' + a.equipment_id + '",' +
		...
		where a.company_id = @i_client_id
			and a.country_code = @i_country_code
			and a.equipment_id like '%' + @p_search_field_1 + '%'
			and a.equipment_oem = @p_search_field_2
			and a.equipment_category = @p_search_field_3
			and a.equipment_type = @p_search_field_4
		return
	end

	if (@p_lov_code_type = 'GET_CALL_TYPE_BASED_ON_ASSET_CONTRACT_STATUS')
	begin
		if (@p_search_field_1 = 'NC')
		begin
			select '' as value_list,
				'{' +
					'"code":"' + a.type_code_value + '",' +
		...
		begin
			...
			where a.company_id = @i_client_id
				and a.country_code = @i_country_code
				and a.link_type = 'AS'
				and a.category_code_value in (@p_search_field_1,'ALL')
			return
		end
