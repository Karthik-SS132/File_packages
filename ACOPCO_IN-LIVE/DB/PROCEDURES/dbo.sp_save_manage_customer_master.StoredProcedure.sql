...
BEGIN
    ...
    declare @p_inputparam_xml xml, @p_location_code nvarchar(10),
			...
			@p_execution_status varchar(10), @p_city_code nvarchar(20),
						...
						select @i_client_id,@i_country_code, ltrim(rtrim(@i_customer_id)),
						   ...
						   case (select applicable from #applicable_custom_fields
							...
							case (select applicable from #applicable_custom_fields
							   ...
							   isnull( (select paramval from #input_params where paramname = 'udf_customer_category') ,''),
								isnull( (select paramval from #input_params where paramname = 'udf_customer_type') ,''),
							    @p_employee_id,
								@i_user_id
	 ...
	 if exists (select 1 from customer
        ...
        begin     
			...
			SET customer_name=@i_customer_name,
				...
				customer_category = isnull( (select paramval from #input_params where paramname = 'udf_customer_category') ,''),
				customer_type = isnull( (select paramval from #input_params where paramname = 'udf_customer_type') ,''),
