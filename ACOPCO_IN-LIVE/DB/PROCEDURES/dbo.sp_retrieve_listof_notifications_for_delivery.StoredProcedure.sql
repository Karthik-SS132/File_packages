    ...
    */
	...
	begin
		...
		where a.company_id = @i_client_id
		  and a.country_code = @i_country_code
		  and notification_id not in (select notification_id from notification_log_delivery)
		  and notification_event_code in ( /*'CALL_CLOSE_CATGB_EMAIL',*/'CALL_CLOSE_SE_TO_CUSTOMER_SMS',
		  'CALL_EMAIL_VR_PDF')
		  and a.notification_event_code not like '%_SECURITY_CODE'
	...
	begin
		...
		and c.notification_mode = d.notification_mode
			...
			begin
				...
				else if @p_notification_template_id like 'ANCILLARY_%'
				begin
					...
					select  @p_recipient_user_id = d.user_id, 
						...
						@p_recipient_email_id = c.email_id, 
						@p_recipient_mobile_no = c.contact_mobile_no
					from ancillary_request_register a, ancillary_request_assignment b, employee c, users d
					where a.company_id = @i_client_id
					  and a.country_code = @i_country_code
					  and a.request_ref_no = @p_call_no
					  and a.company_id = b.company_id
					  and a.country_code = b.country_code
					  and b.request_ref_no = @p_call_no
					  and b.primary_resource_ind = 1
