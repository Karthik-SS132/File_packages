/****** Object:  StoredProcedure [dbo].[sp_update_call_wfeventverb_status_change_se_acopco]    Script Date: 10/17/2023 11:19:16 AM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_update_call_wfeventverb_status_change_se_acopco')
BEGIN
	DROP PROCEDURE [dbo].[sp_update_call_wfeventverb_status_change_se_acopco]
END
 ...
 */
CREATE PROCEDURE [dbo].[sp_update_call_wfeventverb_status_change_se_acopco]
		...
		*/
			...
			begin
			  ...
			  begin
			   ...
			   if exists ( select 1 from call_register
				  ...
				  begin 		
						...
						begin
							...
							begin
							    ...
							    /* Cascade to next visit dates Irrespective of contracts - 24.May.2023 - Chak*/
								/* Recalculate the next visit dates */
								execute sp_update_asset_service_schedule_cascade_dates @i_client_id, @i_country_code, @i_user_id , 
				...
				/* If Commisioning call and machine sl no is available, update installation date */
				if exists ( select 1 from call_register a,asset_master_boa b
				...
				begin
					...
					begin 		
						...
						if exists ( select 1 from asset_master
									where company_id = @i_client_id
									  and country_code = @i_country_code
									  and parent_asset_id = @p_asset_id
									  and installation_date is null)
						begin
		
							select @p_child_asset_id = asset_id from asset_master
									where company_id = @i_client_id
									  and country_code = @i_country_code
									  and parent_asset_id = @p_asset_id
									  and installation_date is null
							...
							begin
								...
								/* Recalculate the next visit dates */
								execute sp_update_asset_service_schedule_cascade_dates @i_client_id, @i_country_code, @i_user_id , 
										@p_child_asset_id, 'INITIALWARRANTY', 0,	@p_update_status OUTPUT, @p_errorno OUTPUT
		...
		begin
			...
			set call_wf_stage_no = @p_to_wf_stage_no,
				...
				closed_on_date = SYSDATETIMEOFFSET(),				
				last_update_id = @i_user_id,
				product_udf_char_1 = isnull((select paramval from #input_params where paramname = 'call_register_product_udf_char_1') , '')
			where company_id = @i_client_id
			  and country_code = @i_country_code
			  and call_ref_no = @i_call_ref_no
		  ...
		  /***** WOL Number insert into call_reister actions***/
		   ...
		   if exists ( select 1 from call_register
			...
			begin
					...
					begin
						...
						begin
							...
							/* Cascade to next visit dates irrespective of contract */
							/* Recalculate the next visit dates */
							execute sp_update_asset_service_schedule_cascade_dates @i_client_id, @i_country_code, @i_user_id , 
									@p_asset_id, @p_contract_doc_no, @p_service_visit_slno,	@p_update_status OUTPUT, @p_errorno OUTPUT
			...
			begin
				...
				where company_id = @i_client_id
				  and country_code = @i_country_code
				  and asset_id = (select parent_asset_id from asset_master
											where company_id = @i_client_id
											  and country_code = @i_country_code
											  and asset_id = @p_asset_id
											  and parent_asset_id != '')
