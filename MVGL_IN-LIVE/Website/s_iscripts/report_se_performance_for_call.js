/* 
 * This file contains invocation code snippets for executing the service report_se_performance_for_call. These code snippets have been organized as three
 * seperate functions. Include these snippet file reference in the html/aspx page or javascript file from where the service report_se_performance_for_call 
 * would be invoked. Substitute USER CONTROL tokens in these snippets, with corresponding user control references of the html/aspx page.
 */

//Code for executing the service report_se_performance_for_call
function executeService_report_se_performance_for_call()
{
    var targetURL =getWebserverpath() + "/mservice/report_se_performance_for_call.aspx";
    
    //Prepare report_se_performance_for_call service request and invoke service 
    var responseXML = executeService(targetURL, prepareServiceRequestData_report_se_performance_for_call());
    ...
    {
       return processServiceResponseData_report_se_performance_for_call(responseXML);
        //return true;
    }
   // return false;
}

//Code for preparing input data for the service report_se_performance_for_call
/* 
 ...
 */
function prepareServiceRequestData_report_se_performance_for_call()
{
    var serviceDetails;
...
//Code for processing output data from the service report_se_performance_for_call
/*
 ...
 */
function processServiceResponseData_report_se_performance_for_call(xmlDoc)
{
    var responseXMLNode = xmlDoc.documentElement.childNodes[0].childNodes;
    var outputparam_detailcount = 1;
    var first_outputparam_detail_item = true;
     var details="";
    details += "<?xml version=\"1.0\" encoding=\"UTF-8\"?><list>";
    ...
    for (var toplevelChildi = 0; toplevelChildi < responseXMLNode.length; toplevelChildi++)
    {
        if (responseXMLNode[toplevelChildi].nodeName == "outputparam_detail")
        {
		  ...
		  details = details+responseXMLNode[toplevelChildi].childNodes[0].lastChild.nodeValue;
        }
    }
