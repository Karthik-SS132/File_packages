DROP PROCEDURE IF EXISTS [dbo].[sp_retrieve_manage_call_register_details]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_retrieve_manage_call_register_details] 
    ...
    @o_call_detail_json1 [uddt_nvarchar_max] OUTPUT, 
    @o_call_detail_json2 [uddt_nvarchar_max] OUTPUT, 
    @o_call_detail_json3 [uddt_nvarchar_max] OUTPUT, 
...
BEGIN
   ...
   */
    ...
    (
		...
		call_json1 nvarchar(max) null,
		call_json2 nvarchar(max) null,
		call_json3 nvarchar(max) null,
		...
		prob_desc nvarchar(max) null,
		addn_desc nvarchar(max) null 
    )
	insert #call_details
	(
		call_ref_no, asset_id, call_status, call_closed_on_date,
		service_contract_doc_no,call_json1, call_json2, call_json3
	)
	select a.call_ref_no, a.asset_id, a.call_status,  a.closed_on_date,
			...
			'"call_wf_stage":"'+
			   ...
			   +'",'+	
				...
				'"charges_currency_code":"'+isnull(charges_currency_code,'INR')+'",'+
				'"charges_currency_code_desc":"'+'INR'+'",'+
				'"charges_gross_amount":"'+cast(isnull(charges_gross_amount,0) as varchar(15))+'",'+
				'"charges_discount_amount":"'+cast(isnull(charges_discount_amount,0) as varchar(15))+'",'+
				'"charges_tax_amount":"'+cast(isnull(charges_tax_amount,0) as varchar(15))+'",'+
				'"charges_net_amount":"'+cast(isnull(charges_net_amount,0) as varchar(15))+'",'+
				'"quotation_ref_no":"'+isnull(a.quotation_ref_no,'')+'",'+
			    '"quotation_ref_date":"'+isnull(CONVERT(varchar(10),a.quotation_ref_date,120),'')+'",'+
			    '"quotation_gross_amount":"'+cast(ISNULL(quotation_gross_amount,0) as varchar(15))+'",'+
			    '"quotation_net_amount":"'+cast(ISNULL(quotation_net_amount,0) as varchar(15))+'",'+
			    '"quotation_discount_amount":"'+cast(ISNULL(quotation_discount_amount,0) as varchar(15))+'",'+
			    '"quotation_tax_amount":"'+cast(ISNULL(quotation_tax_amount,0) as varchar(15))+'",'+
			   ...
			   '"service_contract_doc_desc":"'+
				...
				+'",'+
			   '"source_code_category":"'+ISNULL(a.source_code_category,'')+'",'+
			   '"source_code":"'+ISNULL(a.source_code,'')+'",'+
			   '"expected_closure_date":"'+isnull(convert(varchar(10), a.expected_closure_date, 120),'')+'",'+
  ...
  where a.company_id = @i_client_id
	...
	and a.call_ref_no = @i_call_ref_no
		...
		update #call_details
		set prob_desc = a.problem_description,
			addn_desc = a.additional_information
			 from call_register a
		where a.company_id = @i_client_id
		and a.country_code = @i_country_code
		and a.call_ref_no = @i_call_ref_no
	...
	select @o_call_detail_json1 = call_json1,
			@o_call_detail_json2 = call_json2,
			@o_call_detail_json3 = call_json3+
		   '"prob_desc":"'+ISNULL(prob_desc,'')+'",'+
		   '"addn_desc":"'+ISNULL(addn_desc,'')+'",'+	
