DROP PROCEDURE [dbo].[sp_retrieve_listof_notifications_for_delivery]
GO
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    */
	...
	/* Additional Recipient */
	 ...
	 declare @p_add_recipient_role nvarchar(40), @p_add_recipient_email_id nvarchar(60)
	...
	begin
		...
		and c.notification_mode = d.notification_mode
			...
			else if @p_id_type = 'UR'
			begin
			
				select  @p_add_recipient_role  = '', @p_add_recipient_email_id   = ''
				
			    if @p_notification_template_id  like 'CALL_%' 
				begin
					
					select @p_add_recipient_role = isnull(TEXT,'')
					from #notification_info
					where notification_id = @p_notification_id
					  and localname = '#text'
					  and parentid = (select top(1) id from #notification_info
									  where notification_id = @p_notification_id
										and localname = 'additional_functional_role')
					
					select @p_add_recipient_email_id = isnull(TEXT,'')
					from #notification_info
					where notification_id = @p_notification_id
					  and localname = '#text'
					  and parentid = (select top(1) id from #notification_info
									  where notification_id = @p_notification_id
										and localname = 'additional_email_id')
					
         			select  @p_recipient_user_id = @p_add_recipient_role, 
							@p_recipient_employee_id =  @p_add_recipient_role,
							@p_recipient_name = @p_add_recipient_role,
							@p_recipient_email_id = @p_add_recipient_email_id, 
							@p_recipient_mobile_no = ''
