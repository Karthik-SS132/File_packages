/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_efsr_recepit_form_mvgl_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_efsr_recepit_form_mvgl_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_efsr_recepit_form_mvgl_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_efsr_recepit_form_mvgl_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'efsr_recepit_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'EFSR Receipt Form')
