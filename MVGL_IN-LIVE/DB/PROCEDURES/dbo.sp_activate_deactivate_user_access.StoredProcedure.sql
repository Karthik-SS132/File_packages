   ...
   begin
		...
		begin
		   ...
		   select @p_notification_xml = '<notification_info>'+
			...
			declare @p_user_group nvarchar(10)

		select	@p_user_group = user_group_id
				from users
				  where company_id = @i_client_id
			    and country_code = @i_country_code
			    and user_id = @i_user_id_for_act_deact
			
	 	 if @p_user_group not in ('custowner','custuser')
			begin
			...
			begin
				...
				execute sp_log_new_notification  @i_session_id, @i_user_id  , @i_client_id , 
	   			 @i_locale_id , @i_country_code , 'ACTIVATE_USER_NEW' ,@p_notification_xml, @i_user_id, @p_notification_id OUTPUT
	   		end
