...
DROP FUNCTION IF EXISTS [dbo].[fn_get_client_specific_fields_for_my_activities_mvgl_in]
Go
SET ANSI_NULLS ON
GO
...
CREATE function [dbo].[fn_get_client_specific_fields_for_my_activities_mvgl_in]
(
			...
			(
				...
				(
					case(eventverb_id)
						...
						when 'Warr Form' then 'warr_form'
						when 'Acr Form' then 'acr_form'
						when 'ACD Screw Form' then 'acd_screw_form'
						when 'ACD Recepit Form' then 'acd_recepit_form'
						when 'EFSR Receipt Form' then 'efsr_recepit_form'
						else eventverb_id
					end
				)
