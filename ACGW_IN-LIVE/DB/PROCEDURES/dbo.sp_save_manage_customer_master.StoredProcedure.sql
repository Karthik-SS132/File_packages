IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_customer_master')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_customer_master]
END
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    declare @p_inputparam_xml xml, @p_location_code nvarchar(10),
			...
			@p_execution_status varchar(10),@p_district nvarchar(20),@p_city_code nvarchar(20),
			@p_state_code nvarchar(20)
	...
	set @p_inputparam_xml = CAST(@i_inputparam_udf_xml as XML)
	 ...
	 (paramname nvarchar(50) not null,
	  paramval nvarchar(200) not null
	  )
	  
	  insert #input_params
	  (paramname, paramval)
	  SELECT nodes.value('local-name(.)', 'nvarchar(50)'),
			 nodes.value('(.)[1]', 'nvarchar(200)')
	...
	set @p_district = isnull((select paramval from #input_params where paramname = 'udf_ho_district_code') ,'')	  		  
	
	select @p_city_code = city_code
	from city_master
	where country_code = @i_country_code
	  and city_name = @i_city
