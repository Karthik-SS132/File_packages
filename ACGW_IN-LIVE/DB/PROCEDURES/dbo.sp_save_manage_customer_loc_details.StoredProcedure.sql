IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_customer_loc_details')
BEGIN
...
begin
 ...
 begin
    ...
    (
	 ...
	 udf_analysis_code3,udf_analysis_code4,district_code,
     last_update_id
    )
    select @i_client_id,  @i_customer_id,
	   ...
	   case (select applicable from #applicable_custom_fields
		...
		case (select applicable from #applicable_custom_fields
		   ...
		   isnull( (select paramval from #input_params where paramname = 'udf_ho_district_code') ,''),
			@i_user_id
...
begin
 ...
 begin
	...
	district_code =  isnull( (select paramval from #input_params where paramname = 'udf_ho_district_code') ,''),
