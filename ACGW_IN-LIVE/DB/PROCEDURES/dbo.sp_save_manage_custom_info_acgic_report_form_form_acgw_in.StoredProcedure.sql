/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_acgic_report_form_acgw_in]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_acgic_report_form_acgw_in')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_acgic_report_form_acgw_in]
END
...
CREATE PROCEDURE [dbo].[sp_save_manage_custom_info_acgic_report_form_acgw_in] 
...
BEGIN
	...
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_id', 'acgic_report_form')
	select @i_inputparam_header_xml = json_modify(@i_inputparam_header_xml, '$.screen_name', 'Acg Service Report')
