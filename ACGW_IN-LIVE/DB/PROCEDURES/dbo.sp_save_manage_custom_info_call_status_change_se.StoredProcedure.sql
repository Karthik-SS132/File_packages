/****** Object:  StoredProcedure [dbo].[sp_save_manage_custom_info_call_status_change_se]    Script Date: 3/24/2023 3:24:22 PM ******/
IF EXISTS(SELECT 1 FROM SYS.PROCEDURES WHERE NAME = 'sp_save_manage_custom_info_call_status_change_se')
BEGIN
	DROP PROCEDURE [dbo].[sp_save_manage_custom_info_call_status_change_se]
END
...
BEGIN
	...
	begin
		...
		update call_register	
			...
			if @p_reason_code = '' and @p_channel_id = 'mobile'
			begin
				...
				if @@ROWCOUNT = 0
				begin
					set @i_error_msg = '{"code":"error_update_call_register_for_reason_for_hold"}'
					return
				end
			end
	...
	begin
		update call_register	
			...
			if @@ROWCOUNT = 0
			begin
				set @i_error_msg = '{"code":"error_update_call_register_for_reason_for_releasehold"}'
				return
			end
	end
