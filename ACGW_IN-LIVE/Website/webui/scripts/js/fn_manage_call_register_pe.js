var manage_call_register_pe = {
	constructScreen : function () {
		manage_call_register_pe.variable.custom.datasource_1 = mserviceUtilities.getTransportPagingDataSource({
				applicationName : "mservice",
				serviceName : "retrieve_manage_call_register",
				outputPath : "context/outputparam_detail",
				pageSize : 50,
				inputParameter : {
					p_inputparam_xml : "manage_call_register_pe.customRequirementHandler.getFilterValues()"
				},
				schemaModel : true,
		...
		manage_call_register_pe.variable.custom.datasource_2 = mserviceUtilities.getTransportDataSource({
		...
		if (login_profile.user_group_type == "CU") {
			manage_call_register_pe.variable.custom.customerUserDetails = mserviceUtilities.getTransportDataSource({
					applicationName : "common_modules",
			...
			manage_call_register_pe.variable.custom.customerUserDetails.read();
			if (manage_call_register_pe.variable.custom.customerUserDetails.data()[0] != undefined) {
				manage_call_register_pe.variable.custom.customerUserCustomerID = manage_call_register_pe.variable.custom.customerUserDetails.data()[0].p_value_field_1;
			}
		}
	},
	postConstruct : function () {
		manage_call_register_pe_filterArea = $("#manage_call_register_pe_pane2");
		$("#manage_call_register_pe_content_1 span dt").removeClass("term_one");
		$("#manage_call_register_pe_content_1 span dt").removeClass("term_two");
		$("#manage_call_register_pe_content_1 span dd.colen").html("");
		$("#manage_call_register_pe_content_1").find('dt').css("width","120px");
		$("#manage_call_register_pe_editor").append("<li id='manage_call_register_pe_grid_pager'></li>");
		$("#manage_call_register_pe_grid_pager").append($("#manage_call_register_pe_grid_1").find(".k-grid-pager")).css("width","350px").css("float","right").children().css("border-top-width","0px");
		$("#manage_call_register_pe_grid_1").data("kendoGrid").bind("dataBound", manage_call_register_pe.customRequirementHandler.gridPager);
		if(manage_call_register_pe.variable.custom.autoLoadInd == "true") {
			manage_call_register_pe.variable.custom.grid_1.dataSource.pageSize(50);
		}
		setTimeout(function(){
			manage_call_register_pe_splitter[manage_call_register_pe_filterArea.width() > 0 ? "collapse" : "expand"](manage_call_register_pe_filterArea);
			$("#" + screenID).delegate(webWidgetInitializer.variable.widgetSelector, "change", function () {
				mserviceUtilities.resetFilterIndicator({
					contentID : "manage_call_register_pe_content_1",
					screenID : "manage_call_register_pe",
	...
	initializeWidgets : function () {
		manage_call_register_pe.variable.custom.grid_1 = $("#manage_call_register_pe_grid_1").initializeWGrid({
				screenID : "manage_call_register_pe",
				dataSource : manage_call_register_pe.variable.custom.datasource_1,
				height : 500,
				pageSize : 50,
				filterable : false,
				sortable : true,
		...
		$("#manage_call_register_pe_contextMenu").kendoContextMenu({
			orientation: "vertical",
			target: "[data-widget-type = 'w_link'][data-link-type = 'actions']",
		});
		$("#manage_call_register_pe_editor").kendoMenu();		
		manage_call_register_pe_splitter = $("#manage_call_register_pe_splitter").kendoSplitter({
			panes: [
				{ collapsible: true, size: 330 ,resizable: false },
				{ }				
			],
			resize: function(e) {
				manage_call_register_pe.customRequirementHandler.gridPager();
			}
		}).data("kendoSplitter");
	},
	...
	buttonEventHandler : {
		crud_btn_click : function (element, event) {
			if (manage_call_register_pe.variable.custom.crudIndicator == "R") {
				manage_call_register_pe.variable.custom.grid_1.dataSource._skip = 0;
				manage_call_register_pe.variable.custom.grid_1.dataSource.pageSize(50);
			} else {
				if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_manage_call_register_pe_edit.js", "../../s_iscripts/save_manage_call_register.js"])) {
					webNavigationController.gotoNextScreen({
						screenID : "manage_call_register_pe",
						nextScreenID : "manage_call_register_pe_edit",
		...
		misc_btn_click : function (element, event){
			if(manage_call_register_pe.variable.custom.miscID == "filters"){
				manage_call_register_pe_splitter[manage_call_register_pe_filterArea.width() > 0 ? "collapse" : "expand"](manage_call_register_pe_filterArea);
			}
			if(manage_call_register_pe.variable.custom.miscID == "clear_filters"){
				mserviceUtilities.resetInputparamXML({
					contentID : "manage_call_register_pe_content_1",
					screenID : "manage_call_register_pe",
					matchCondition : ["_filter"]
				});
				$("#manage_call_register_pe_filters_btn .k-i-filter").css("color","black");
			}
		},
		feature_btn_click : function (element, event) {
			if (manage_call_register_pe.variable.custom.selectedFeatureID == "manage_user_attachments") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
						if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_manage_user_attachments.js"])) {
							...
							manage_user_attachments.variable.custom.project_id = manage_call_register_pe.variable.custom.selectedRecord.call_no;
							manage_user_attachments.variable.custom.template_id = "";
							manage_user_attachments.variable.custom.task_id = "0";
							manage_user_attachments.variable.custom.project_status_defaultValue = manage_call_register_pe.variable.custom.selectedRecord.call_status_desc;
							webNavigationController.gotoNextScreen({
								screenID : "manage_call_register_pe",
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "view_calendar") {
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "manage_call_resource_update") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "manage_call_sparepart_update") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "track_employee_geolocation") {
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "generate_service_coupon") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
						manage_call_register_pe.variable.custom.selectedRecord = manage_call_register_pe.variable.custom.grid_1.dataSource.getByUid(manage_call_register_pe.variable.custom.grid_1.select().data("uid"));
						if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_generate_service_coupon.js"])) {
							manage_call_register_pe.variable.custom.visit_template_id = mserviceUtilities.getTransportDataSource({
									applicationName : "common_modules",
									serviceName : "retrieve_listof_values_for_searchcondition",
									inputParameter : {
										p_inputparam_xml : "'<inputparam><lov_code_type>VISITTEMPLATEID</lov_code_type><search_field_1>" + manage_call_register_pe.variable.custom.selectedRecord.call_no + "</search_field_1></inputparam>'"
									},
								});
							manage_call_register_pe.variable.custom.visit_template_id.read();
							if (manage_call_register_pe.variable.custom.visit_template_id.data().length != 0) {
								generate_service_coupon.variable.custom.document_type = "AssetServiceVisitSchedule";
								generate_service_coupon.variable.custom.document_template = manage_call_register_pe.variable.custom.visit_template_id.data()[0].visittemplateid,
								...
								generate_service_coupon.variable.custom.retrieve_service_inputparam += getXmlString("<call_no>" + manage_call_register_pe.variable.custom.selectedRecord.call_no + "</call_no>");
								generate_service_coupon.variable.custom.retrieve_service_inputparam += getXmlString("<asset_id>" + manage_call_register_pe.variable.custom.selectedRecord.asset_id + "</asset_id>");
								generate_service_coupon.variable.custom.retrieve_service_inputparam += "</p_inputparam_xml>";
								generate_service_coupon.variable.custom.retrieve_service_inputparam += "</inputparam>";
								...
								generate_service_coupon.variable.custom.output_file_name = manage_call_register_pe.variable.custom.visit_template_id.data()[0].visitreportno;
								...
								if (generate_service_coupon.variable.custom.generate_pdf_document_dataSource_1.data()[0].p_update_status == "SP001") {
									webNavigationController.gotoNextScreen({
										screenID : "manage_call_register_pe",
										fieldID : "manage_call_register_pe_child_window",
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "generate_expense_document") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
						manage_call_register_pe.variable.custom.selectedRecord = manage_call_register_pe.variable.custom.grid_1.dataSource.getByUid(manage_call_register_pe.variable.custom.grid_1.select().data("uid"));
						if (mserviceUtilities.loadJSScripts(["../../s_iscripts/generate_expense_document.js"])) {
							if (confirm("Are you sure do you want to generate expense document for this call ?")) {
								...
								returnValue = executeService_generate_expense_document({
										...
										p_call_jo_project_ref_no : manage_call_register_pe.variable.custom.selectedRecord.call_no
									});
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "manage_call_stockout") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
						if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_manage_call_stockout.js", "../../s_iscripts/update_call_wfeventverb_status_change.js"])) {
							...
							allowedWorkflowArray = mserviceUtilities.getNextWorkflowEventVerbs({
									...
									eventVerb : "MATLISSUE",
									requestCategory : manage_call_register_pe.variable.custom.selectedRecord.call_category,
									requestType : manage_call_register_pe.variable.custom.selectedRecord.call_type,
									fromStage : manage_call_register_pe.variable.custom.selectedRecord.workflow_stage_no,
									fromStatus : manage_call_register_pe.variable.custom.selectedRecord.call_status
								});
							...
							if (allowedWorkflowArray.length != 0) {
								...
								manage_call_register_pe.variable.custom.selectedCallLoggedDate = mserviceUtilities.getDateObject({
										dateString : manage_call_register_pe.variable.custom.selectedRecord.created_on_date,
										hourString : manage_call_register_pe.variable.custom.selectedRecord.created_on_hour,
										minuteString : manage_call_register_pe.variable.custom.selectedRecord.created_on_minute
									});
								manage_call_register_pe.variable.custom.selectedActualStartDate = mserviceUtilities.getDateObject({
										dateString : manage_call_register_pe.variable.custom.selectedRecord.act_start_on_date,
										hourString : manage_call_register_pe.variable.custom.selectedRecord.act_start_on_hour,
										minuteString : manage_call_register_pe.variable.custom.selectedRecord.act_start_on_minute
									});
								manage_call_register_pe.variable.custom.selectedScheduledStartDate = mserviceUtilities.getDateObject({
										dateString : manage_call_register_pe.variable.custom.selectedRecord.sch_start_on_date,
										hourString : manage_call_register_pe.variable.custom.selectedRecord.sch_start_on_hour,
										minuteString : manage_call_register_pe.variable.custom.selectedRecord.sch_start_on_minute
									});
								manage_call_register_pe.variable.custom.selectedScheduledFinishDate = mserviceUtilities.getDateObject({
										dateString : manage_call_register_pe.variable.custom.selectedRecord.sch_finish_on_date,
										hourString : manage_call_register_pe.variable.custom.selectedRecord.sch_finish_on_hour,
										minuteString : manage_call_register_pe.variable.custom.selectedRecord.sch_finish_on_minute
									});
								webNavigationController.gotoNextScreen({
									screenID : "manage_call_register_pe",
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "manage_call_stockin") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
					if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
			...
			} else if (manage_call_register_pe.variable.custom.selectedFeatureID == "generate_roc_document") {
				if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
		...
		workflow_btn_click : function (element, event) {
			var allowedWorkflowArray;
			if (manage_call_register_pe.variable.custom.grid_1.select().length != 0) {
				/*manage_call_register_pe.variable.custom.validateCallAccess = mserviceUtilities.getTransportDataSource({
						applicationName : "common_modules",
						serviceName : "retrieve_listof_values_for_searchcondition",
						outputPath : "context/outputparam",
						inputParameter : {
							p_inputparam_xml : "<inputparam><lov_code_type>'VALIDATE_CALL_ACCESS'</lov_code_type><search_field_1>$manage_call_register_pe.variable.custom.selectedRecord.call_no</search_field_1><search_field_2>'WEB'</search_field_2></inputparam>"
						},
						screenID : "manage_call_register_pe",
					});
				manage_call_register_pe.variable.custom.validateCallAccess.read();
				if (manage_call_register_pe.variable.custom.validateCallAccess.data()[0].access_ind == "1") {
					if (confirm("Your update may be overwritten by the update from the mobile.Do you wish to continue ?")) {*/
						if (manage_call_register_pe.customRequirementHandler.validateAccess(element)) {
							if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_manage_call_register_pe_wfeventverb_status_change.js", "../../s_iscripts/update_call_wfeventverb_status_change.js"])) {
								...
								allowedWorkflowArray = mserviceUtilities.getNextWorkflowEventVerbs({
										transactionType : "CALL",
										eventVerb : manage_call_register_pe.variable.custom.selectedWorkflowEventVerb,
										requestCategory : manage_call_register_pe.variable.custom.selectedRecord.call_category,
										requestType : manage_call_register_pe.variable.custom.selectedRecord.call_type,
										fromStage : manage_call_register_pe.variable.custom.selectedRecord.workflow_stage_no,
										fromStatus : manage_call_register_pe.variable.custom.selectedRecord.call_status
	...
	linkEventHandler : {
		actions_link_click : function (element,event){
			$("#manage_call_register_pe_contextMenu").data("kendoContextMenu").open(element);
			manage_call_register_pe.variable.custom.contextChildElement = $('#manage_call_register_pe_contextMenu').children();
			for (i = 0; i < manage_call_register_pe.variable.custom.contextChildElement.length; i++){
				if (manage_call_register_pe.variable.custom.contextChildElement[i].dataset.buttonGroup == 'workflow'){
					$('#' + screenID + '_'+ manage_call_register_pe.variable.custom.contextChildElement[i].dataset.buttonRole + '_btn').hide();
				}
			}
			manage_call_register_pe.variable.custom.selectedRecord = manage_call_register_pe.variable.custom.grid_1.dataSource.getByUid(manage_call_register_pe.variable.custom.grid_1.select().data("uid"));
			...
			for (workFlowLength = 0; workFlowLength < allowedWorkflowArray.length; workFlowLength ++){
				...
				for (i = 0; i < manage_call_register_pe.variable.custom.contextChildElement.length; i++){
					if (manage_call_register_pe.variable.custom.contextChildElement[i].dataset.buttonRole == currentFeature ){
						$('#' + screenID + '_'+ manage_call_register_pe.variable.custom.contextChildElement[i].dataset.buttonRole + '_btn').show();
						break;
					}
				} 
			}
			$("#manage_call_register_pe_contextMenu").show();
			var columnData = $.grep(manage_call_register_pe.variable.custom.contextChildElement, function (data) {
			...
			$("#manage_call_register_pe_contextMenu").css("columns", columnCount.toString()).css("width", "min-content");
		},
		quotation_reference_link_click : function (element, event) {
			if (mserviceUtilities.loadJSScripts(["../scripts/js/fn_manage_quotation_master_spares.js"])) {
				var nextScreenProperties = $.grep(access_profile.user_functional_access, function (element, index) {
					return element.child_screen_id == "manage_quotation_master_spares";
				});
				manage_quotation_master_spares.variable.custom.linkRefInd = "true";
				webNavigationController.gotoNextScreen({
					screenID : "manage_call_register_pe",
					nextScreenID : "manage_quotation_master_spares",
					nextScreenName : nextScreenProperties[0].child_feature_display_label
				});
				$("#manage_call_register_pe").remove();
				...
				$("#manage_quotation_master_spares_quotation_number_filter").setVal($(element).text());
				manage_quotation_master_spares.variable.custom.grid_1.dataSource.read();
			} else {
				alert("Sorry. This feature is unavailable. Please contact your support desk.");
				return false;
			}
	...
	customRequirementHandler : {
		gridPager : function () {
			$("#manage_call_register_pe_grid_1").find(".k-grid-content").css("height", 498 - $("#manage_call_register_pe_grid_1").find(".k-grid-header").height());
		},
		getFilterValues : function () {
			return $("#manage_call_register_pe_content_1").getInputparamXML({
				screenID : "manage_call_register_pe",
				matchCondition : ["_filter"]
			});
		},
		...
		validateAccess : function (buttonId) {
			if (login_profile.user_group_id == "clientadmn" || login_profile.user_group_id == "sadmn") {
				...
				var callObject = manage_call_register_pe.variable.custom.selectedRecord;
				manage_call_register_pe.variable.custom.datasource_2.read();
				if ((callObject.assigned_to_emp_id == undefined || callObject.assigned_to_emp_id == "") || (callObject.assigned_to_emp_id == login_profile.emp_id || callObject.assignee_mapped_to_emp_id == login_profile.emp_id)) {
					var validateAccessArray = $.grep(manage_call_register_pe.variable.custom.datasource_2.data(), function (element, index) {
					...
					if (validateAccessArray.length == 0) {
						validateAccessArray = $.grep(manage_call_register_pe.variable.custom.datasource_2.data(), function (element, index) {
		...
		getExportConfig : function (gridId) {
			if(gridId == "manage_call_register_pe_grid_1_export_btn"){
				return {
					type : "csv",
					template : "manage_call_register_pe",
					service : "sp_retrieve_manage_call_register",
					request : "<signature><i_inputparam_xml>" + manage_call_register_pe.customRequirementHandler.getFilterValues().replace("</inputparam>","<skip></skip><take></take></inputparam>") + "</i_inputparam_xml><o_retrieve_status></o_retrieve_status></signature>",
					length : manage_call_register_pe.variable.custom.grid_1.dataSource.data().length
				};
			}
		}
	},
	variable : {
		standard : {
			...
			exportConfiguration : {
				...
				content : [{
						exportType : "grid",
						fieldId : "manage_call_register_pe_grid_1",
