var report_generic_data = {
	...
	refreshScreen: function () {
		...
		if (report_generic_data.variable.custom.crudIndicator != "D") {
			report_generic_data.variable.custom.grid_1.dataSource.pageSize(10);
		}
	},
				...
				if ($("#report_generic_data_report_code").getVal() == "report_generic_data_hr_report" || $("#report_generic_data_report_code").getVal() == "report_generic_data_finance_report") {
					...
					if(soft_dataSource.data().length != 0){
						...
						columns = [{
								...
								field : "customer_contact_mobile_no",
								...
								field : "attitude_service",
								title : "Overall Feedback",
								width : 200
							}, {
								field : "changeAdditionCmnts",
								title : "Comments",
								width : 200
							}		
						];
						...
						for(var i = 0; i < required_dataSource.length; i++) {
							...
							for(var j = 0; j < required_dataSource[i].items.length; j++) {
								...
								obj["customer_contact_mobile_no"] = required_dataSource[i]["items"][j]["items"][0].customer_contact_mobile_no;
								obj["customer_contact_name"] = required_dataSource[i]["items"][j]["items"][0].customer_contact_name;
								obj["customer_contact_email_id"] = required_dataSource[i]["items"][j]["items"][0].customer_contact_email_id;
							}
							actual_dataSource.push(obj);
						}
					...
					recordCounter;
					report_generic_data.variable.custom.grid_1.dataSource.pageSize(10);
					if(report_generic_data.variable.custom.grid_1.dataSource.data().length != 0){
				...
				if ($("#report_generic_data_report_code").getVal() == "report_generic_data_hr_report" || $("#report_generic_data_report_code").getVal() == "report_generic_data_finance_report") {
					...
					for (var i = 0; i < data.length; i++) {
						...
						for (var j in obj.fields) {
							var call = report_generic_data.customRequirementHandler.setData_in_export1(data[i][j], j);
							cells.push(call);
						}
						rows.push({
							cells: cells
					...
					kendo.saveAs({
						...
						fileName: "FileName",
						proxyURL: "./fmt/kendoProxy/kendoProxyExport.aspx"
					});
				}else if ($("#report_generic_data_report_code").getVal() == "report_generic_data_customer_feedback_report" ) {
					// first row
					var obj = {
						...
						cells : [{
								...
								value : "Customer Contact Email Id",
								vAlign : "center",
								hAlign : "center",
								background : "#e5e3e3",
								fontSize : 12,
	...
	customRequirementHandler : {
		setData_in_export : function (data, j) {
			if($("#report_generic_data_report_code").getVal() == "report_generic_data_customer_feedback_report" || "report_generic_data_fsr_report"|| "report_generic_data_spm_report"){
			...
			else if($("#report_generic_data_report_code").getVal() == "report_generic_data_hr_report" || "report_generic_data_finance_report"){
				...
				else if ($("#report_generic_data_report_code").getVal() == "report_generic_data_hr_report") {
						var obj = {
		...
		setData_in_export1 : function (data, j) {
			if($("#report_generic_data_report_code").getVal() == "report_generic_data_hr_report" || "report_generic_data_finance_report"){
				if (typeof(data) == "string") {
					if (j == "employee_name") {
						var obj = {
							value: data,
