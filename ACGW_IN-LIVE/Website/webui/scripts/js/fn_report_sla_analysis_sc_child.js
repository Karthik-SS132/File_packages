function fn_report_sla_analysis_sc_child()
{
	if(screen_id_for_retrieve != "")
	{
		...
		for(var i =0;i<length ;i++)
		{
			...
			if(filters_xml.childNodes[0].childNodes[i].hasChildNodes() == true)
			{
				...
				eval("report_sla_analysis_sc_child_"+filters_xml.childNodes[0].childNodes[i].nodeName+"_value = "+ "'"+value +"'");
			}
			else
			{
				var node_value = "ALL";
				eval("report_sla_analysis_sc_child_"+filters_xml.childNodes[0].childNodes[i].nodeName+"_value = "+ "'"+node_value +"'");
			}
		}
	}
	else
	...
	AddCustomFilterForReport("report_sla_analysis_sc_child");
	ApplyConfiguredLabelsForReport("report_sla_analysis_sc_child");
	$('#report_sla_analysis_sc_child_set').on("click",function()
	{
		screen_id_for_retrieve  = 'report_sla_analysis_sc_child_header_1';
		...
		for(var i=0;i<header_filtered_value.length;i++)
		{
			...
			{
				 Nodevalue = value[0];
				 eval("report_sla_analysis_sc_"+header_filtered_value[i].nodeName+".value("+"'"+Nodevalue+"'"+")");
			}
			else
			{
				 Nodevalue = value;
			...
			{
				eval("report_sla_analysis_sc_"+header_filtered_value[i].nodeName+'.enable(false)');
			}
			else
			{
	...
	$('#report_sla_analysis_sc_child_reset').on("click",function()
	{
		var header_filtered_value=[];
		var input_xml = GetInputParamXML("report_sla_analysis_sc_child_header_1");
		...
		for(var i=0;i<header_filtered_value.length;i++)
		{
			eval("report_sla_analysis_sc_child_"+header_filtered_value[i].nodeName+'.value("ALL")');
			//eval("report_sla_analysis_sc_"+header_filtered_value[i].nodeName+'.value("ALL")');
		//	eval("report_sla_analysis_sc_"+header_filtered_value[i].nodeName+'.enable(true)');
		}
	});

	$('#report_sla_analysis_sc_child_cancel').on("click",function()
	{
		child_window.close();
	});	
}
