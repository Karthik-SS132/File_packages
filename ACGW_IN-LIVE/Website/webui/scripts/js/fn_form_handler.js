var form_handler = {
	constructScreen: function () {
		$.ajax({
			url: mserviceUtilities.getWebserverpath() + "webui/configuration/ui/ui_form_handler_" + form_handler.variable.template + "_" + login_profile.client_id + "_" + login_profile.country_code + ".txt",
			async: false,
			cache: false
		}).done(function (data) {
		...
		$.ajax({
			url: mserviceUtilities.getWebserverpath() + "webui/configuration/label/label_form_handler_" + form_handler.variable.template + "_" + login_profile.client_id + "_" + login_profile.country_code + "_" + login_profile.locale_id + ".txt",
			async: false,
			cache: false
		}).done(function (data) {
		...
		$.ajax({
			url: mserviceUtilities.getWebserverpath() + "content_store/" + login_profile.client_id + "/" + login_profile.country_code + "/msvCache/cache_controller_com.selfservit.mservice_" + login_profile.client_id + "_" + login_profile.country_code + ".json",
			async: false,
			cache: false
		}).done(function (data) {
		...
		$("#form_handler_" + form_handler.variable.template + "_submit_btn").click(function(e) {
			if(form_handler.variable.validator["form_handler"].validate()){
				...
				$.ajax({
					async: false,
					url: mserviceUtilities.getWebserverpath() + "/common/components/QueueManager/QueueUploadWeb.aspx?client_id=" + login_profile.client_id + "&country_code=" + login_profile.country_code + "&user_id=" + login_profile.user_id + "&file_path=queue&file_name=" + mserviceUtilities.getDateString(new Date(),"yyyyMMddHHmmssfff") + "_" +  login_profile.user_id + "_attachment.txt",
					method: "POST",
					dataType: "json",
					contentType: "application/json; charset=utf-8",
					data: JSON.stringify({
						"url": mserviceUtilities.getWebserverpath() + "/common/components/convertHtml2Pdf/convertHtml2PDFOffline.aspx",
						...
						"input": JSON.stringify({
							"context": {
								...
								"inputparam": {
									...
									"p_output_file_path": "content_store/" + login_profile.client_id + "/" + login_profile.country_code + "/call_attachments/" + form_handler.variable.transNo + "/",
									"p_output_file_name": form_handler.variable.transNo + "_" + login_profile.emp_id + "_" + mserviceUtilities.getDateString(new Date(), "ddMMyy") + "_" + form_handler.variable.template + ".pdf",
									"form_version": "web"
								}
							}
						})
					...
					success: function () {
						var counter,
						inputparamxml = "<inputparam><online_offline_ind>F</online_offline_ind><event_date>" + mserviceUtilities.getDateString(new Date(), "yyyy-MM-dd") + "</event_date><event_hour>" + mserviceUtilities.getDateString(new Date(), "HH") + "</event_hour><event_minute>" + mserviceUtilities.getDateString(new Date(), "mm") + "</event_minute>",
						inputparamWidgets = $("#form_handler").find("[id$='_savedetails']"),
						widgetId;
						for (counter = 0; counter < inputparamWidgets.length; counter++) {
							widgetId = $(inputparamWidgets[counter]).attr("id").replace("form_handler_" + form_handler.variable.template + "_", "").replace("_savedetails", "");
							if($(inputparamWidgets[counter]).formHandlerGetVal() != undefined){
								inputparamxml += "<" + widgetId + ">" + getXmlString($(inputparamWidgets[counter]).formHandlerGetVal().replace(/\n/g, " ")) + "</" + widgetId + ">";
							}
						};
						...
						$.ajax({
							...
							contentType: "application/json; charset=utf-8",
							data: JSON.stringify({
								"url": mserviceUtilities.getWebserverpath() + "/JSONServiceEndpoint.aspx?appName=common_modules&serviceName=save_file_to_attachment_master&path=context/outputparam",
								"key": "my_attachments_list",
								"subkey": form_handler.variable.transNo,
								"input": JSON.stringify({
									"context": {
										"sessionId": login_profile.guid_val,
										...
										"country_code": login_profile.country_code,
										"inputparam_header": {
											p_project_id: form_handler.variable.transNo,
											p_task_id: form_handler.variable.taskID,
											p_attach_level_ind: form_handler.variable.attachInd,
											p_inputparam_xml: inputparamxml,
											p_file_category: "P",
											p_file_type: "P",
											p_file_name: form_handler.variable.transNo + "_" + login_profile.emp_id + "_" + mserviceUtilities.getDateString(new Date(), "ddMMyy") + "_" + form_handler.variable.template + ".pdf",
											p_closure_report_ind: "1"
							...
							success: function () {
								...
								$("#"+webNavigationController.getCurrentScreenID()+"_form_handler_window").data("kendoWindow").destroy();
							},
							failure: function () {
								$("#spinner").hide();
	...
	customRequirementHandler : {
		...
		getHtml: function (iObj) {
			...
			html += (((iObj.wType != "mLoop") || (iObj.wType == "mLoop" && iObj.loopIndex == "0")) ? ("<div style='display: grid;' id = '" + ((iObj.wType == "mLoop") ? (iObj.id.substring(0, iObj.id.lastIndexOf("-"))) : (iObj.id)) + "_group' name = '" + ((iObj.wType == "mLoop") ? (iObj.id.substring(0, iObj.id.lastIndexOf("-"))) : (iObj.id)) + "_group'>") : (""));
			...
			if (iObj.wType == "mParentgroup"){
				...
				html += "<div style='width: 38%; float: left; padding: 0.1%; word-wrap: break-word; margin-left:" + ((iObj.parent != "" || iObj.loop != "") ? ("") : ("3")) + "%;'><label style='font-family: " + 'Georgia' + ", Times New Roman, Times, serif; font-size: 15px;' id = '" + iObj.id + "_lbl' name = '" + iObj.id + "_lbl' data-widget-type = 'mLabel'" + addnlAttr + "></label>&nbsp;&nbsp;&nbsp;</div>";
				html += "<div style='overflow: hidden; padding-bottom: 1%;'><span style='display: table;'><textarea style='width:201px; height: 80px; padding : 1px;' " + addnlAttr + " id = '" + iObj.id + "'  name = '" + iObj.id + "' data-widget-type='" + iObj.wType + "'></textarea></span>";
				html += "<span style='margin-left: 152px;' data-for = '" + iObj.id + "' class = 'k-invalid-msg'></span></div>";
			...
			for (index in iObj.fields) {
				...
				if (iObj.fields[index].wType == "mParentgroup") {
					...
					if(form_handler.variable.custom.cacheController[iObj.fields[index].dataSrc] != undefined){
						var cacheConfig = form_handler.variable.custom.cacheController[iObj.fields[index].dataSrc];
						if(cacheConfig != undefined && cacheConfig.code != undefined && cacheConfig.inputXml != undefined){
							cacheDataSrc = mserviceUtilities.getTransportPagingDataSource({
						...
						$("#" + id).kendoDropDownList({
							dataSource: {
								transport: {
									read: {
										url: mserviceUtilities.getWebserverpath() + "JSONServiceEndpoint.aspx?appName=" + iObj.fields[index].dataSrc.applicationName + "&serviceName=" + iObj.fields[index].dataSrc.serviceName + "&path=" + iObj.fields[index].dataSrc.outputPath,
										dataType: "json",
										type: "POST",
									...
									parameterMap: function (data, type) {
										return '{"context":{"sessionId":"' + login_profile.guid_val + '","userId":"' + login_profile.user_id + '","client_id":"' + login_profile.client_id + '","locale_id":"' + login_profile.locale_id + '","country_code":"'+ login_profile.country_code +'","inputparam":{"p_inputparam_xml":"' + iObj.fields[index].dataSrc.inputParameter.p_inputparam_xml + '"}}}';
									}
								}
							},
							autoBind: true,
									...
									if (event.filter.value != "") {
										...
										if($("#" + fieldID).attr("data-inputparam") != undefined){
											...
											this.dataSource.transport.parameterMap = function (data, type) {
												return '{"context":{"sessionId":"' + login_profile.guid_val + '","userId":"' + login_profile.user_id + '","client_id":"' + login_profile.client_id + '","locale_id":"' + login_profile.locale_id + '","country_code":"'+ login_profile.country_code +'","inputparam":{"p_inputparam_xml":"' + inputString + '"}}}';
											};
            ...
            for (index = 0; index < fieldList.length; index++) {
                ...
                if ($(fieldList[index]).attr("data-widget-type") == "mAttachment") {
                    ...
                    if ($(fieldList[index]).attr("data-widget-type") == "mDatebox") {
                        if (mLoop != undefined) {
                            inputparamObj[root][parseInt(mLoopIndex)] += getXmlString(kendo.toString($(fieldList[index]).formHandlerGetVal(), "yyyy-MM-dd"));
                        } else {
                            inputparamObj[root] += getXmlString(kendo.toString($(fieldList[index]).formHandlerGetVal(), "yyyy-MM-dd"));
                        }
                    } else if ($(fieldList[index]).attr("data-widget-type") == "mTimebox") {
                        if (mLoop != undefined) {
                            inputparamObj[root][parseInt(mLoopIndex)] += getXmlString(kendo.toString($(fieldList[index]).formHandlerGetVal(), "HH:mm"));
                        } else {
                            inputparamObj[root] += getXmlString(kendo.toString($(fieldList[index]).formHandlerGetVal(), "HH:mm"));
                        }
                    } else if ($(fieldList[index]).attr("data-widget-type") == "mMultiselect") {
                        if (mLoop != undefined) {
                            inputparamObj[root][parseInt(mLoopIndex)] += getXmlString($(fieldList[index]).formHandlerGetVal().join());
                        } else {
                            inputparamObj[root] += getXmlString($(fieldList[index]).formHandlerGetVal().join());
                        }
						...
						kendo.drawing.drawDOM($("#" + fieldList[index].id + "_surface")).then(function(group) {
							...
							if (mLoop != undefined) {
								inputparamObj[root][parseInt(mLoopIndex)] += getXmlString(data);
							} else {
								inputparamObj[root] += getXmlString(data);
							}
						});
                    } else {
                        if (mLoop != undefined) {
                            inputparamObj[root][parseInt(mLoopIndex)] += getXmlString($(fieldList[index]).formHandlerGetVal().toString());
                        } else {
                            inputparamObj[root] += getXmlString($(fieldList[index]).formHandlerGetVal().toString());
                        }
                    };
                    if (mLoop != undefined) {
                        inputparamObj[root][parseInt(mLoopIndex)] += "</" + $(fieldList[index]).attr("id").substring(0, $(fieldList[index]).attr("id").lastIndexOf("-")).replace(iObj.scrID + "_", "") + ">";
...
$.fn.extend({
	formHandlerApplyConfiguredRules : function () {
		...
		if (currentScreenID != undefined) {
			...
			if (grepReturnValue == undefined) {
				ruleFileUrl = mserviceUtilities.getWebserverpath() + "webui/configuration/rule/rule_form_handler_" + form_handler.variable.template + "_" + login_profile.client_id + "_" + login_profile.country_code + ".txt";
				$.ajax({
					url : ruleFileUrl,
					async : false,
					cache : false
	...
	formHandlerSetVal : function (value) {
		if (value != undefined) {
			if ($(this).attr("data-widget-type") == "mTextbox" || $(this).attr("data-widget-type") == "mTextarea"  || $(this).attr("data-widget-type") == "mGeotag") {
				$(this).val(value);
			} else if ($(this).attr("data-widget-type") == "mNumerictextbox") {
