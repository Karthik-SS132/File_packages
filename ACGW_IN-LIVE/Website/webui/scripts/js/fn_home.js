var home = {
	...
	updateHeaderFooter : function () {
		var homeHeaderCompanyLogoImg = new Image();
		$("#home_logo img").attr("src", "../../common/images/" + login_profile.client_id + "/" + login_profile.country_code + "/" + "company_logo.png");	
		homeHeaderCompanyLogoImg.src = '/common/images'+'/'+login_profile.client_id+'/'+login_profile.country_code+'/'+'company_logo.png';
		homeHeaderCompanyLogoImg.onload = function() {
			var homeHeaderCompanyLogoCanvas = document.createElement("canvas");
			homeHeaderCompanyLogoCanvas.width =140;
			homeHeaderCompanyLogoCanvas.height=80;
			var ctx = homeHeaderCompanyLogoCanvas.getContext("2d");
			var hRatio = homeHeaderCompanyLogoCanvas.width  / homeHeaderCompanyLogoImg.width    ;
			var vRatio =  homeHeaderCompanyLogoCanvas.height / homeHeaderCompanyLogoImg.height  ;
			var ratio  = Math.min( hRatio, vRatio );
			var centerShift_x = ( homeHeaderCompanyLogoCanvas.width - homeHeaderCompanyLogoImg.width * ratio ) / 2;
			var centerShift_y = ( homeHeaderCompanyLogoCanvas.height - homeHeaderCompanyLogoImg.height * ratio ) / 2;  
			ctx.clearRect(0,0,homeHeaderCompanyLogoCanvas.width, homeHeaderCompanyLogoCanvas.height);
			ctx.drawImage(homeHeaderCompanyLogoImg, 0, 0, homeHeaderCompanyLogoImg.width, homeHeaderCompanyLogoImg.height, centerShift_x, centerShift_y, homeHeaderCompanyLogoImg.width * ratio, homeHeaderCompanyLogoImg.height * ratio); 
			var dataurl = homeHeaderCompanyLogoCanvas.toDataURL();
			$('#home_logo img').attr('src',dataurl);
		};
	...
	validateFeatureAccess : function () {
		...
		if (accessibleFeatures.length == 1) {
			...
			$('#breadcrumb').show();
		} else if (accessibleFeatures.length > 1) {
			$("#menu").hide();
