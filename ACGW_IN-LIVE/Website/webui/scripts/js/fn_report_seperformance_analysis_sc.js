function fn_report_seperformance_analysis_sc_loadScripts()
{
	var jsScriptsToLoad = [];
	jsScriptsToLoad = [ 
						"./webui/scripts/js/fn_report_seperformance_analysis_sc_add_filter.js",
					  ];
...
function fn_report_seperformance_analysis_sc()
{

	screenChangeInd='';
	tat_uom = "";
	$("#report_seperformance_analysis_sc_tabstrip").kendoTabStrip();
	$("#report_seperformance_analysis_sc_heading_title").text(MenuTitle);
	p_inputparam_xml="";
	...
	var report_seperformance_analysis_sc_datasource_1_temp =  new kendo.data.DataSource({data : temp_array, pageSize : 10});		
	report_seperformance_analysis_sc_grid_1 = InitializeKendoGrid(
	{
		sectionID: "report_seperformance_analysis_sc_detail_1",
		toolbar: false,
		dataSource: report_seperformance_analysis_sc_datasource_1_temp,
		height: 400,
		pageSize: 10,
		resizable:true,
		sortable:true,
	...
	$("#report_seperformance_analysis_sc_grid_1 .k-grid-content").css("height","332px");
	
	var report_seperformance_analysis_sc_datasource_2_temp =  new kendo.data.DataSource({data : temp_array, pageSize : 10});		
	report_seperformance_analysis_sc_grid_2 = InitializeKendoGrid(
	{
		sectionID: "report_seperformance_analysis_sc_detail_2",
		//toolbar: "report_seperformance_analysis_sc_grid_1_toolbar_template",
		dataSource: report_seperformance_analysis_sc_datasource_2_temp,
		height: 400,
		pageSize: 10,
		resizable:true,
		sortable:true,
	...
	ApplyConfiguredLabelsForReport("report_seperformance_analysis_sc");		
	...
	//AddCustomFilterForReport("report_seperformance_analysis_sc");
	ApplyConfiguredLabelsForReport("report_seperformance_analysis_sc");
	
	screen_id_for_retrieve = "";
	$('#report_seperformance_analysis_sc_retrieve_btn').on("click",function()
	{
		...
		for(var i =0;i<length ;i++)
		{
			...
			{
				eval("report_seperformance_analysis_sc_"+filters_xml.childNodes[0].childNodes[i].nodeName+'.enable(false)');
			}
		} 
		retrieveData(xml);
	});
	$('#report_seperformance_analysis_sc_reset_filter').on("click",function()
	{
		var header_filtered_value=[];
		var input_xml = GetInputParamXML("report_seperformance_analysis_sc_header_1");
		...
		for(var i=0;i<header_filtered_value.length;i++)
		{
			...
			$(".display_description[data-for='report_seperformance_analysis_sc_"+header_filtered_value[i].nodeName+"']").text('');
		}
		
		report_seperformance_analysis_sc_grid_1.dataSource.data([]);
		report_seperformance_analysis_sc_grid_2.dataSource.data([]);
		report_seperformance_analysis_sc_datasource_1.data([]);
		report_metrics_data = [];
		chart_array= [];
		series_type_array=[];
		groupdata=[];
	...
	$('#report_seperformance_analysis_sc_add_filter').on('click', function() 
	{
		child_window =  InitializeKendoWindow(
			{
				fieldID: 'report_seperformance_analysis_sc_window',
				windowTitle: 'Add Filters',
				windowHeight: 450,
				windowWidth: 900,
				screenID: 'report_seperformance_analysis_sc_add_filter'
			});
	});		
	
	
	chart_type_data =[{code:"column",description:"Column"},{code:"bar",description:"Bar"},{code:"line",description:"Line"}];
	summary_and_series_data= SummaryAndSeriesData("report_seperformance_analysis_sc");
	summary_by_data = summary_and_series_data.summary;
	series_by_data = summary_and_series_data.series;
	
	report_seperformance_analysis_sc_chart_type = InitializeKendoDropDownList({
		fieldID: "report_seperformance_analysis_sc_chart_type",
	...
	report_seperformance_analysis_sc_chart_type.value(chart_type_data[0].code);
	report_seperformance_analysis_sc_chart_type.options.optionLabel = false;
	report_seperformance_analysis_sc_chart_type.refresh();
	$('[data-for =report_seperformance_analysis_sc_chart_type]').text(chart_type_data[0].description);
	
	report_seperformance_analysis_sc_chart_series = InitializeKendoDropDownList({
		fieldID: "report_seperformance_analysis_sc_chart_series",
	...
	report_seperformance_analysis_sc_chart_series.value(series_by_data[0].value);	
	report_seperformance_analysis_sc_chart_series.options.optionLabel = false;
	report_seperformance_analysis_sc_chart_series.refresh();
	$('[data-for =report_seperformance_analysis_sc_chart_series]').text(series_by_data[0].text);
	
	report_seperformance_analysis_sc_chart_summary = InitializeKendoDropDownList({
		fieldID: "report_seperformance_analysis_sc_chart_summary",
	...
	report_seperformance_analysis_sc_chart_summary.value(summary_by_data[0].value);
	report_seperformance_analysis_sc_chart_summary.options.optionLabel = false;
	report_seperformance_analysis_sc_chart_summary.refresh();
	$('[data-for =report_seperformance_analysis_sc_chart_summary]').text(summary_by_data[0].text);
	
	report_seperformance_analysis_sc_datasource_1_schema_model = GetDataSourceSchema("report_seperformance_analysis_sc","report_seperformance_analysis_sc_datasource_1");
	report_seperformance_analysis_sc_datasource_1 = new kendo.data.DataSource(
	{
		transport: 
		{
			read: 
			{
				...
				url: GetTransportUrl("mservice", "report_se_performance_for_call", "context/outputparam_detail"),
		...
		schema: 
		{
			...
			model: report_seperformance_analysis_sc_datasource_1_schema_model,
			
			parse: function (response)
			{
				return GetDescriptionFieldsForDataSource(response, report_seperformance_analysis_sc_datasource_1);
			}
		}
	});	
	report_seperformance_analysis_sc_grid_1.dataSource.options.schema = report_seperformance_analysis_sc_datasource_1.options.schema;
	...
	$("#report_seperformance_analysis_sc").delegate("span.call_reference_view", "click", function()
	{
		if(LoadJSScripts(["./webui/scripts/js/fn_manage_call_register_edit.js", "../../s_iscripts/retrieve_manage_call_register_details.js"]))
		{
			...
			selected_call_register_record = report_seperformance_analysis_sc_grid_1.dataSource.getByUid(report_seperformance_analysis_sc_grid_1.select().data("uid")).call_ref_no;
			$.get('manage_call_register_edit.html', function(data)
			{							
				$("#report_seperformance_analysis_sc").hide();
			...
			manage_call_register = {
					variable : {
						custom : {
							crudIndicator : "V",
							selectedRecord : {
								call_no :  report_seperformance_analysis_sc_grid_1.dataSource.getByUid(report_seperformance_analysis_sc_grid_1.select().data("uid")).call_ref_no
							}
						}
					}
				};
				webNavigationController.gotoNextScreen({
					screenID : "report_seperformance_analysis_sc",
	...
	DefineFavouritesForFilters("report_seperformance_analysis_sc");
	
	var exportAccess = $.grep(access_profile.user_functional_access, function (element, index) {
		return element.child_screen_id == $("#report_seperformance_analysis_sc").attr("id");
	})[0];
	if (exportAccess != undefined) {
		if (exportAccess.export_access == "true") {
			$("#report_seperformance_analysis_sc_chart").append("<button class= 'k-button' id= 'report_seperformance_analysis_sc_chart_export_btn' style='position:absolute; right:0; cursor:pointer; top: 2%; margin-right:1%;' data-widget-type = 'w_button' data-button-group = 'export' data-button-role = 'chart'>Chart Export</button>");
		} 
	};	
}
...
function manage_display_chart()
{
	...
	{
		series_type_array[0].type = $("#report_seperformance_analysis_sc_chart_type").val();
	}
	var uom = "";
	$("#report_seperformance_analysis_sc_chart").kendoChart({
...
function refresh() 
{
		var chart = $("#report_seperformance_analysis_sc_chart").data("kendoChart");
		series = chart.options.series;
		type = $("#report_seperformance_analysis_sc_chart_type").val();
	  //stack = $("#stack").prop("checked");
		length = series.length;
		if( $("#report_seperformance_analysis_sc_chart_series").val() != "No series")
		{
...
{
	if(report_seperformance_analysis_sc_datasource_1.data().length != 0)
	{
		...
		for(var i =0;i<length ;i++)
		{
			...
			if(filters_xml.childNodes[0].childNodes[i].hasChildNodes() == true)
			{
				...
				if(filters_xml.childNodes[0].childNodes[i].childNodes[0].nodeValue !="ALL")
				{
					...
					var DataRole  = $("#report_seperformance_analysis_sc_" +filters_xml.childNodes[0].childNodes[i].nodeName ).data("role");
		...
		{
			report_seperformance_analysis_sc_datasource_1.filter(organogram);
			organogram_data = report_seperformance_analysis_sc_datasource_1.view();
		...
		{
			report_seperformance_analysis_sc_datasource_1.filter([]);
			organogram_data = report_seperformance_analysis_sc_datasource_1.view();
			organogram_datasource = new kendo.data.DataSource({data:organogram_data});organogram_datasource.read();
		}
		...
		//report_seperformance_analysis_sc_datasource_1.data(header_filtered_array)
		report_seperformance_analysis_sc_grid_1.dataSource.data(header_filtered_array);
		
		modified_datasource = new kendo.data.DataSource({data:header_filtered_array});modified_datasource.read();
		var category = $("#report_seperformance_analysis_sc_chart_summary").val();
		var series = $("#report_seperformance_analysis_sc_chart_series").val();
		...
		if( series == "No series")
		{
			...
			modified_datasource.group({field:category,aggregates:[{field:"tat",aggregate:"sum"}]});
			 grouped_data = modified_datasource.view();
			  if(grouped_data.length != 0)
				{
					$.each(grouped_data,function(index,element){
						tat_uom = element.items[0]['tat_uom'];
						if(report_seperformance_analysis_sc_chart_summary.dataItem().display == "description")
						{
						...
						else if(report_seperformance_analysis_sc_chart_summary.dataItem().display == "code")
						{
						...
						if(tat_uom == "Hours")
						{
							final_data =parseInt(element.aggregates.tat.sum);
						}
						else
						{
							final_data =(element.aggregates.tat.sum).toFixed(2);
						}
						chart_array.push({
					...
					report_seperformance_analysis_sc_grid_2.dataSource.data(summarizedGridData);
				}
		}
		else
		{
				...
				$.each(x_axis,function(index1,element1){
					$.each(y_axis,function(index2,element){
						...
						if(arr.length!= 0)
						{
							...
							ds.aggregate([{field:"tat",aggregate:"sum"}]);
							if(ds.aggregates().tat.sum != 0)
							{
								if(tat_uom == "Hours")
								{
									final_data =parseInt(ds.aggregates().tat.sum);
								}
								else
								{
									final_data =(ds.aggregates().tat.sum).toFixed(2);
								}
							}}
						tat_uom = element1.items[0]['tat_uom'];
...
function retrieveData(xml)
{
		...
		report_seperformance_analysis_sc_datasource_1.data([]);
		$("#spinner").show();
		report_seperformance_analysis_sc_datasource_1.read();
		report_se_metric=report_seperformance_analysis_sc_datasource_1.data (); 
		no_jo=report_se_metric.length;
		total_sc='[{"field":"no_sc"}]';
		total_sc_array=$.parseJSON(total_sc);


}
function fn_report_seperformance_analysis_sc_PreImport()
{
	return true;
}
function fn_report_seperformance_analysis_sc_PostImport()
{
	return true;
}
function fn_report_seperformance_analysis_sc_PreExport()
{
	exportConfiguration = {
		mode:'multiple',
		content:[{
			exportType:"grid",
			fieldId:"report_seperformance_analysis_sc_grid_1",
			dispalyLabel:"Data"
		},
		{
			exportType:"grid",
			fieldId:"report_seperformance_analysis_sc_grid_2",
			dispalyLabel:"Summarized Data"
		},
		{
			exportType:"chart",
			fieldId:"report_seperformance_analysis_sc_chart",
			dispalyLabel:"Chart"
		}]
	};
	return exportConfiguration;
}
function fn_report_seperformance_analysis_sc_PostExport()
{
	return true;
}
function fn_report_seperformance_analysis_sc_PrePrint()
{
	printConfiguration = {
		mode:'single',
		content:[{
...
function fn_report_seperformance_analysis_sc_PostPrint()
{
	return true;
}
function fn_report_seperformance_analysis_sc_getExportConfig(gridId) {
	if(gridId == "report_seperformance_analysis_sc_grid_1_export_btn"){
		return {
			type : "csv",
			template : "report_seperformance_analysis_sc",
			service : "sp_report_se_performance_for_call",
			request : "<signature><i_inputparam_xml>" + p_inputparam_xml + "</i_inputparam_xml><o_retrieve_status></o_retrieve_status></signature>",
			length : report_seperformance_analysis_sc_datasource_1._pristineData.length
		};
	}
};	