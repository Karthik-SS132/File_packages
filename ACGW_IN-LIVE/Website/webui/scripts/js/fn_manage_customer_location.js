var manage_customer_location = {
	constructScreen : function () {
		if (webNavigationController.getPreviousScreenID() != "home_container") {
			...
			manage_customer_location.variable.custom.customer_name_filter_defaultValue = manage_customer_master.variable.custom.selectedRecord.cust_name;
		}
	...
	buttonEventHandler : {
		...
		feature_btn_click : function (element, event) {
			...
			} else if (manage_customer_location.variable.custom.selectedFeatureID == "manage_customer_gst") {
				if (manage_customer_location.variable.custom.selectedRecord != undefined) {
			...
			if (manage_customer_location.variable.custom.selectedFeatureID == "manage_customer_contacts_edit") {
				if (manage_customer_location.variable.custom.selectedRecord != undefined) {
	...
	customRequirementHandler : {
		...
		getExportConfig : function (gridId) {
			if(gridId == "manage_customer_location_grid_1_export_btn"){
				return {
					type : "csv",
					template : "manage_customer_location",
					service : "sp_retrieve_manage_customer_loc_list",
					request : "<signature><i_inputparam_xml>" + manage_customer_location.customRequirementHandler.getFilterValues().replace("</inputparam>","<skip>0</skip><take>" + manage_customer_location.variable.custom.grid_1.dataSource.data()[0].total + "</take></inputparam>") + "</i_inputparam_xml><o_retrieve_status></o_retrieve_status></signature>",
					length : manage_customer_location.variable.custom.grid_1.dataSource.data().length
				};
			}
		}
	},
