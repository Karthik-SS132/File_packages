/* Mservice Common Scripts - Version 1.0.05 - 10/09/2021 */
...
var webConfigurationEngine = {
		...
		for (buttonCounter = 0; buttonCounter < exportButton.length; buttonCounter++) {
			exportAccess = $.grep(access_profile.user_functional_access, function (element, index) {
					if (tempParam != undefined)
						return element.child_screen_id == tempParam;
					else
	...
	createFieldGroup : function (fieldGroupObject) {
		...
		descriptionElement = "";
		if (fieldGroupObject.type == "w_dropdownlist" || fieldGroupObject.type == "w_combobox" || fieldGroupObject.type == "w_searchbox" ||
		fieldGroupObject.type == "w_combotextbox"){
